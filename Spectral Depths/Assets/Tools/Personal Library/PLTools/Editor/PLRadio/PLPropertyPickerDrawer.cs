...
{
	[CustomPropertyDrawer(typeof(PLPropertyPicker), true)]
	public class PLPropertyPickerDrawer : PropertyDrawer
	{
		...
		protected virtual void DrawLevelProgressBar(Rect position, float level, Color frontColor, Color negativeColor, PropertyPickerViewData viewData)
		{
			...
			float progressWidth = PLMaths.Remap(progressLevel, 0f, 1f, 0f, fullProgressWidth);
		...
		protected virtual Type GetPropertyType(SerializedProperty property, PropertyPickerViewData viewData)
		{
			...
			PLProperty tempProperty;

			tempProperty = PLProperty.FindProperty(viewData._propertiesList[viewData._selectedPropertyIndex], property.FindPropertyRelative("TargetComponent").objectReferenceValue as Component, null, property.FindPropertyRelative("TargetObject").objectReferenceValue as ScriptableObject);

...
{
	#if UNITY_EDITOR
	[CustomEditor(typeof(PLAudioAnalyzer), true)]
	[CanEditMultipleObjects]
	public class PLAudioAnalyzerEditor : Editor
	{
		public bool Active;
		...
		protected Color _normalBarColor = PLColors.Orange;
		protected Color _normalNormalizedBarColor = PLColors.Aqua;
		protected Color _peakColor = PLColors.Yellow;
		protected Color _activePeakColor = Color.white;
		protected Color _amplitudeColor = PLColors.DarkOrange;
		protected Color _normalizedAmplitudeColor = PLColors.Aquamarine;
		protected Color _spectrumColor = PLColors.HotPink;
		...
		{
			if ((Beats == null) || (target as PLAudioAnalyzer).Beats == null)
			{
				return;
			}

			float length = (target as PLAudioAnalyzer).Beats.Length;
			if (length <= 0)
			{
				return;
			}
			...
			for (int i = 0; i < length; i++)
			{
				...
				{
					// draw front bar 
					_beatColor = (target as PLAudioAnalyzer).Beats[i].BeatColor;
					_beatColor.a = (target as PLAudioAnalyzer).Beats[i].CurrentValue;
					_rect.x = boxX;
					_rect.y = boxY;
					_rect.width = beatsBoxSquareSize;
					_rect.height = beatsBoxSquareSize;
		...
		protected virtual void DrawBandVisualization()
		{
			...
			if ((BandPeaks != null) && (BandPeaks.arraySize == _numberOfBands))
			{
				for (int i = 0; i < _numberOfBands; i++)
				{
					...
					_axisOrigin.y = _boxBottomY - PLMaths.Remap(peak, 0f, 1f, 0f, _maxColumnHeight);
			...
			// amplitude cursors
			_columnHeight = PLMaths.Remap(Amplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			...
			_columnHeight = PLMaths.Remap(BufferedAmplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			...
			for (int i = 0; i < _numberOfBands; i++)
			{
				if (Active)
				{
					...
					_columnHeight = PLMaths.Remap(bandLevel, 0f, 1f, 0f, _maxColumnHeight);
		...
		protected virtual void DrawBandVisualizationNormalized()
		{
			...
			// amplitude cursors
			_columnHeight = PLMaths.Remap(NormalizedAmplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			_positionX = _boxPosition.x - _externalMargin / 4;
			_positionY = _boxBottomY - _columnHeight;
			...
			_columnHeight = PLMaths.Remap(NormalizedBufferedAmplitude.floatValue, 0f, 1f, 0f, _maxColumnHeight);
			_positionX = _boxPosition.x + _boxSize.x - _externalMargin / 4;
			_positionY = _boxBottomY - _columnHeight;
		...
		protected virtual void DrawRawSpectrum()
		{
			...
			if (Active)
			{
				...
				for (int i = 1; i < RawSpectrum.arraySize - 1; i++)
				{
					...
					float xPosition = _spectrumBoxPosition.x + _externalMargin + PLMaths.Remap(i, 0, RawSpectrum.arraySize, 0f, _spectrumBoxSize.x - _externalMargin * 2);
			...
			else
			{
				...
				{
					float xPosition = _spectrumBoxPosition.x + _externalMargin + PLMaths.Remap(i, 0, points, 0f, _spectrumBoxSize.x - _externalMargin * 2);
					float yPosition = _spectrumBoxPosition.y + _spectrumBoxSize.y / 2;
		...
		{
			if ((target as PLAudioAnalyzer).Mode != PLAudioAnalyzer.Modes.AudioSource)
			{
				return;
			}
			...
			if (!PeaksPasted.boolValue)
			{
				if ((PeaksSaver.Peaks == null) || (PeaksSaver.Peaks.Length == 0))
				{
					...
					{
						(target as PLAudioAnalyzer).FindPeaks();
					}
				}
				else
				{
					...
					if (!Application.isPlaying)
					{
						...
						{
							(target as PLAudioAnalyzer).PastePeaks();
						}
					}                    
				}
			}            
			...
			{
				(target as PLAudioAnalyzer).ClearPeaks();
			}        
		}
	}
	#endif

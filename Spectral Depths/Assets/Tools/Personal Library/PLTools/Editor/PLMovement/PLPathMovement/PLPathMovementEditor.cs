...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[CustomEditor(typeof(PLPathMovement),true)]
	[InitializeOnLoad]
	public class PLPathMovementEditor : Editor 
	{		
		public PLPathMovement pathMovementTarget
		{
			get
			{
				return (PLPathMovement)target;
			}
		}
		...
		public override void OnInspectorGUI()
		{
			...
			if (pathMovementTarget.AccelerationType == PLPathMovement.PossibleAccelerationType.AnimationCurve)
			{
		...
		protected virtual void OnSceneGUI()
		{
			...
			PLPathMovement t = (target as PLPathMovement);
			...
			for (int i=0;i<t.PathElements.Count;i++)
			{
				...
				var fmh_65_57_638356820176652132 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle(oldPoint,.5f,new Vector3(.25f,.25f,.25f),Handles.CircleHandleCap);

...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	public class PLMeshToPolygonCollider2D : MonoBehaviour
	{
		/// <summary>
		...
		private static Vector3[] MeshFilterToVectors(MeshFilter meshFilter)
		{
			...
			List<PLGeometry.PLEdge> boundaryPath = PLGeometry.GetEdges(meshFilter.sharedMesh.triangles).FindBoundary().SortEdges();
		...
		private static bool ValidateMesh(MeshFilter meshFilter)
		{
			...
			{
				Debug.LogWarning("[PLMeshToPolygonCollider2D] " 
		...
		/// </summary>
		[MenuItem("Tools/Spectral Depths/Collisions/Generate PolygonCollider2D", false, 601)]
		public static void GeneratePolygonCollider2DMenu()
		{
			Transform activeTransform = Selection.activeTransform;
			if (activeTransform == null)
			{
				Debug.LogWarning("[PLMeshToPolygonCollider2D] You need to select a gameobject first.");
				return;
			}

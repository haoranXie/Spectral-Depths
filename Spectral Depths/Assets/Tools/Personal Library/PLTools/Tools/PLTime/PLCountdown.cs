...
namespace SpectralDepths.Tools
{
	...
	[AddComponentMenu("Spectral Depths/Tools/Time/PLCountdown")]
	public class PLCountdown : PLMonoBehaviour
	{
		[Serializable]
		/// <summary>
		/// A class to store floor information
		/// </summary>
		public class PLCountdownFloor
		{
		...
		public enum PLCountdownDirections { Ascending, Descending }

		[PLInspectorGroup("Countdown", true, 18)]
		[PLInformation("You can define the bounds of the countdown (how much it should count down from, and to how much, the format it should be displayed in (standard Unity float ToString formatting).", SpectralDepths.Tools.PLInformationAttribute.InformationType.Info, false)]
		...
		[PLInspectorGroup("Display", true, 19)]
		...
		[PLEnumCondition("FormatMethod", (int)FormatMethods.Explicit)]
		...
		[PLEnumCondition("FormatMethod", (int)FormatMethods.Choices)]
		...
		[PLInspectorGroup("Settings", true, 20)]
		[PLInformation("You can choose whether or not the countdown should automatically start on its Start, at what frequency (in seconds) it should refresh (0 means every frame), and the countdown's speed multiplier " +
		               ...
		               "Bind events here to trigger them when the countdown reaches its To destination, or every time it gets refreshed.", SpectralDepths.Tools.PLInformationAttribute.InformationType.Info, false)]
		...
		[PLInspectorGroup("Floors", true, 21)]
		/// a list of floors this countdown will evaluate and trigger if met
		public List<PLCountdownFloor> Floors;
        
		[PLInspectorGroup("Events", true, 22)]
		...
		[PLInspectorButton("DebugSetNewCurrentTime")] 
		...
		protected virtual void Initialization()
		{
			...
			Direction = (CountdownFrom > CountdownTo) ? PLCountdownDirections.Descending : PLCountdownDirections.Ascending;
			...
			foreach (PLCountdownFloor floor in Floors)
			{
		...
		{
			if (Direction == PLCountdownDirections.Descending)
			{
		...
		protected virtual void UpdateText()
		{
			if (Time.time - _lastRefreshAt > RefreshFrequency)
			{
				if (_text != null)
				{
					...
					{
						newText = PLTime.FloatToTimeString(CurrentTime, Hours, Minutes, Seconds, Milliseconds);
					}                    
		...
		protected virtual void CheckForEnd()
		{
			...
			bool endReached = (Direction == PLCountdownDirections.Ascending) ? (CurrentTime >= CountdownTo) : (CurrentTime <= CountdownTo);
			...
			if (endReached)
			{
				...
				if (PingPong)
				{
					Direction = (Direction == PLCountdownDirections.Ascending) ? PLCountdownDirections.Descending : PLCountdownDirections.Ascending;
					_countdowning = true;
		...
		{
			Direction = Direction == PLCountdownDirections.Descending
				? PLCountdownDirections.Ascending
				: PLCountdownDirections.Descending;

...
{
	/// <summary>
	/// A class used to bind a slider to a PLDebugMenu
	/// </summary>
	public class PLDebugMenuItemSlider : MonoBehaviour
	{
		...
		public void ValueChangeCheck()
		{
			...
			SliderValue = PLMaths.Remap(TargetSlider.value, 0f, 1f, RemapZero, RemapOne);
		...
		{
			PLDebugMenuSliderEvent.Trigger(SliderEventName, value, PLDebugMenuSliderEvent.EventModes.FromSlider);
		}
		...
		{
			if ((eventMode == PLDebugMenuSliderEvent.EventModes.SetSlider)
			    && (sliderEventName == SliderEventName))
			{
				_valueSetThisFrame = true;
				TargetSlider.value = PLMaths.Remap(value, RemapZero, RemapOne, 0f, 1f);
				UpdateValue(value);
			}
		}

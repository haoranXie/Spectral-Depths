...
{	
	...
	public static class PLDebug 
	{
						...
						.SelectMany(
							m => m.GetTypes().SelectMany(
								n => n.GetMethods(BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public)
									.Where(o => o.GetCustomAttribute<PLDebugLogCommandAttribute>() != null))).ToArray();
				}
		...
		public static void DebugLogCommand(string command)
		{
			...
			string commandFirst = SString.UppercaseFirst(splitCommand[0]);
			...
			{ 
				...
				PLDebugLogCommandArgumentCountAttribute argumentAttribute = commandInfo.GetCustomAttributes<PLDebugLogCommandArgumentCountAttribute>(true).FirstOrDefault();
		...
		private static void LogCommand(string command, string color)
		{
			...
			LogHistory.Add(item);
			PLDebugLogEvent.Trigger(new DebugLogItem(null, "", Time.frameCount, Time.time, 0, false));
		}
		...
		public static string LogHistoryText
		{
			get
			{
				...
				for (int i = 0; i < LogHistory.Count; i++)
				{
					...
					log.Append("<color=#f9a682>[" + PLTime.FloatToTimeString(LogHistory[i].Time, false, true, true, true) + "]</color> ");
		...
		//public static PLConsole _console;
		public static PLDebugOnScreenConsole _console;
		private const string _debugConsolePrefabPath = "PLDebugOnScreenConsole";
                
		/// <summary>
		/// Instantiates a PLConsole if there isn't one already, and adds the message in parameter to it.
		/// </summary>
		/// <param name="message">Message.</param>
		...
		/// <summary>
		/// Instantiates a PLConsole if there isn't one already, and displays the label in bold and its value next to it.
		/// </summary>
		...
		public static void InstantiateOnScreenConsole(int fontSize=25)
		{
			...
			if (_console == null)
			{
				...
				_console = (PLDebugOnScreenConsole) GameObject.FindObjectOfType(typeof(PLDebugOnScreenConsole));
			}
			...
			{	
				...
				loaded.name = "PLDebugOnScreenConsole";
				_console = loaded.GetComponent<PLDebugOnScreenConsole>();                
			}
		}
		...
		public static void SetOnScreenConsole(PLDebugOnScreenConsole newConsole)
		{
			_console = newConsole;
		}
		...
		public static void DrawRectangle (Rect rectangle, Color color)
		{
			...
			PLDebug.DrawRectangle (pos, color, scale); 
		}	

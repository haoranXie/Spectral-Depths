...
namespace SpectralDepths.Tools
{
	...
	/// You can create your own classes that implement this interface, or use the PLPersistent class that comes with this package
	...
	/// </summary>
	public class PLPersistenceManager : PLPersistentSingleton<PLPersistenceManager>, PLEventListener<PLGameEvent>
	{
		[Header("Persistence")]
		...
		public string PersistenceID = "PLPersistency";
		...
		[PLInspectorButton("SaveToMemory")]
		public bool SaveToMemoryButton;
		[PLInspectorButton("LoadFromMemory")]
		public bool LoadFromMemoryButton;
		[PLInspectorButton("SaveFromMemoryToFile")]
		public bool SaveToFileButton;
		[PLInspectorButton("LoadFromFileToMemory")]
		public bool LoadFromFileButton;
		[PLInspectorButton("DeletePersistencyFile")]
		...
		public static string _saveFolderName = "PLTools/";
		...
		#region SAVE_AND_LOAD
			...
			public virtual void SaveToMemory()
			{
				...
				PLPersistenceSceneData sceneData = new PLPersistenceSceneData();
				...
				PLPersistenceEvent.Trigger(PLPersistenceEventType.DataSavedToMemory, PersistenceID);
			}
			...
			public virtual void LoadFromMemory()
			{
				...
				if (!SceneDatas.TryGetValue(_currentSceneName, out PLPersistenceSceneData sceneData))
				{
					return;
				}
				...
				PLPersistenceEvent.Trigger(PLPersistenceEventType.DataLoadedFromMemory, PersistenceID);
			}
			...
			{
				PLPersistenceManagerData saveData = new PLPersistenceManagerData();
				saveData.PersistenceID = PersistenceID;
				saveData.SaveDate = DateTime.Now.ToString();
				saveData.SceneDatas = SceneDatas;
				PLSaveLoadManager.Save(saveData, DetermineSaveName(), _saveFolderName);
				
				PLPersistenceEvent.Trigger(PLPersistenceEventType.DataSavedFromMemoryToFile, PersistenceID);
			}
			...
			{
				PLPersistenceManagerData saveData = (PLPersistenceManagerData)PLSaveLoadManager.Load(typeof(PLPersistenceManagerData), DetermineSaveName(), _saveFolderName);
				if ((saveData != null) && (saveData.SceneDatas != null))
				{
					SceneDatas = new DictionaryStringSceneData();
					SceneDatas = saveData.SceneDatas;	
				}
				PLPersistenceEvent.Trigger(PLPersistenceEventType.DataLoadedFromFileToMemory, PersistenceID);
			}
		...
		#region EVENTS
			...
			/// <summary>
			/// When we get a PLEvent, we filter on its name and invoke the appropriate methods if needed
			/// </summary>
			...
			/// <summary>
			/// On enable, we start listening for PLGameEvents
			/// </summary>
			...
			/// <summary>
			/// On enable, we stop listening for PLGameEvents
			/// </summary>

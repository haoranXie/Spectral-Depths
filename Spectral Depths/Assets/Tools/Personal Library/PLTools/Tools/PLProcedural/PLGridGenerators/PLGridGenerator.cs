...
namespace SpectralDepths.Tools
{
	public class PLGridGenerator
	{
		...
		public static int[,] TilemapToGrid(Tilemap tilemap, int width, int height)
		{
			if(tilemap == null)
			{
				Debug.LogError("[PLGridGenerator] You're trying to convert a tilemap into a grid but didn't specify what tilemap to convert.");
				return null;
			}
			...
			for (int i = 0; i < width; i++)
			{
				for (int j = 0; j < height; j++)
				{
					currentPosition.x = i;
					currentPosition.y = j;
					currentPosition += PLTilemapGridRenderer.ComputeOffset(width-1, height-1);
		...
		/// <returns></returns>
		public static int[,] ApplySafeSpots(int[,] grid, List<PLTilemapGeneratorLayer.PLTilemapGeneratorLayerSafeSpot> safeSpots)
		{
			foreach (PLTilemapGeneratorLayer.PLTilemapGeneratorLayerSafeSpot safeSpot in safeSpots)
			{

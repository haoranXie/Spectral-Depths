...
namespace SpectralDepths.Tools
{
	...
	[AddComponentMenu("Spectral Depths/Tools/GUI/PLProgressBar")]
	public class PLProgressBar : PLMonoBehaviour
	{
		public enum PLProgressBarStates {Idle, Decreasing, Increasing, InDecreasingDelay, InIncreasingDelay }
		...
		[PLInspectorGroup("Fill Settings", true, 11)]
		...
		[PLCondition("SetInitialFillValueOnStart", true)]
		[Range(0f,1f)]
		...
		[PLInspectorGroup("Foreground Bar Settings", true, 12)]
		...
		[PLCondition("LerpForegroundBar", true)]
		...
		[PLInspectorGroup("Delayed Bar Decreasing", true, 13)]
		...
		[PLCondition("LerpDecreasingDelayedBar", true)]
		...
		[PLInspectorGroup("Delayed Bar Increasing", true, 18)]
		...
		/// the speed at which to lerp the delayed bar
		[Tooltip("the speed at which to lerp the delayed bar")]
		[PLCondition("LerpIncreasingDelayedBar", true)]
		...
		[PLCondition("ChangeColorWhenBumping", true)]
		...
		[PLInspectorGroup("Events", true, 16)] 
		...
		[PLCondition("DisplayTotal", true)]
		public string TotalSeparator = " / ";

		[PLInspectorGroup("Debug", true, 15)]
		...
		[PLInspectorButton("DebugUpdateBar")]
		public bool DebugUpdateBarButton;
		[PLInspectorButton("DebugSetBar")]
		public bool DebugSetBarButton;
		[PLInspectorButton("Bump")]
		public bool TestBumpButton;
		[PLInspectorButton("Plus10Percent")]
		public bool Plus10PercentButton;
		[PLInspectorButton("Minus10Percent")]
		public bool Minus10PercentButton;
        
		[PLInspectorGroup("Debug Read Only", true, 19)]
		...
		protected PLProgressBarStates CurrentState = PLProgressBarStates.Idle;
		...
		public virtual void UpdateBar(float currentValue,float minValue,float maxValue) 
		{
			...
			_newPercent = PLMaths.Remap(currentValue, minValue, maxValue, MinimumBarFillValue, MaximumBarFillValue);
			...
			if (CurrentState != PLProgressBarStates.Idle)
			{
				if ((CurrentState == PLProgressBarStates.Decreasing) ||
				    (CurrentState == PLProgressBarStates.InDecreasingDelay))
				{
				...
				if ((CurrentState == PLProgressBarStates.Increasing) ||
				    (CurrentState == PLProgressBarStates.InIncreasingDelay))
				{
					if (_newPercent <= BarTarget)
					{
						StopCoroutine(_coroutine);
		...
		{
			float newPercent = PLMaths.Remap(currentValue, minValue, maxValue, 0f, 1f);
			SetBar01(newPercent);
		}
		...
		public virtual void SetBar01(float newPercent)
		{
			...
			newPercent = PLMaths.Remap(newPercent, 0f, 1f, MinimumBarFillValue, MaximumBarFillValue);
			...
			CurrentState = PLProgressBarStates.Idle;
		}
		...
		protected virtual void UpdateBars()
		{
			...
			if (_direction < 0)
			{
				...
				CurrentState = PLProgressBarStates.Decreasing;
			...
			else // if the value is increasing
			{
				...
				DelayedBarIncreasingProgress = newFill;
				CurrentState = PLProgressBarStates.Increasing;
		...
		protected virtual float ComputeNewFill(bool lerpBar, float barSpeed, float barDuration, AnimationCurve barCurve, float delay, float lastPercent, out float t)
		{
			...
			if (lerpBar)
			{
				...
				delta = PLMaths.Remap(timeSpent, 0f, duration, 0f, 1f);
		...
		protected virtual void SetBarInternal(float newAmount, Transform bar, Image image, Vector2 initialSize)
		{
			...
			switch (FillMode)
			{
				...
				case FillModes.Width:
					...
					float newSizeX = PLMaths.Remap(newAmount, 0f, 1f, 0, initialSize.x);
				...
				case FillModes.Height:
					...
					float newSizeY = PLMaths.Remap(newAmount, 0f, 1f, 0, initialSize.y);

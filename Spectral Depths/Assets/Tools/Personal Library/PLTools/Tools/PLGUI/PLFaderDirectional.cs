...
namespace SpectralDepths.Tools
{
	...
	[AddComponentMenu("Spectral Depths/Tools/GUI/PLFaderDirectional")]
	public class PLFaderDirectional : MonoBehaviour, PLEventListener<PLFadeEvent>, PLEventListener<PLFadeInEvent>, PLEventListener<PLFadeOutEvent>, PLEventListener<PLFadeStopEvent>
	{
		...
		{
			PLFadeEvent.Trigger(DefaultDuration, 1f, DefaultTween, ID, IgnoreTimescale, this.transform.position);
		}
		...
		{
			PLFadeInEvent.Trigger(1f, DefaultTween, ID, IgnoreTimescale, this.transform.position);
		}
		...
		{
			PLFadeOutEvent.Trigger(1f, DefaultTween, ID, IgnoreTimescale, this.transform.position);
		}
		...
		protected virtual void Fade()
		{
			...
			{
				_newPosition = PLTween.Tween(currentTime, _fadeStartedAt, endTime, _fromPosition, _toPosition, _currentCurve);
		...
		protected virtual IEnumerator StartFading(bool fadingIn, float duration, PLTweenType curve, int id,
			bool ignoreTimeScale, Vector3 worldPosition)
		{
			...
			if (InitialDelay > 0f)
			{
				yield return PLCoroutine.WaitFor(InitialDelay);
			}
			...
			_newPosition = PLTween.Tween(0f, 0f, duration, _fromPosition, _toPosition, _currentCurve);
			_rectTransform.anchoredPosition = _newPosition;
		}

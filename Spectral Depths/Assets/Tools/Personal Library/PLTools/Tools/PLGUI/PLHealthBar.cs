...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/GUI/PLHealthBar")]
	public class PLHealthBar : MonoBehaviour 
	{
		...
		[PLInformation("Add this component to an object and it'll add a healthbar next to it to reflect its health level in real time. You can decide here whether the health bar should be drawn automatically or use a prefab.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		[PLInformation("Select a prefab with a progress bar script on it. There is one example of such a prefab in Common/Prefabs/GUI.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		/// the prefab to use as the health bar
		[Tooltip("the prefab to use as the health bar")]
		public PLProgressBar HealthBarPrefab;

		[Header("Existing PLProgressBar")]
		/// the PLProgressBar this health bar should update 
		[Tooltip("the PLProgressBar this health bar should update")]
		public PLProgressBar TargetProgressBar;

		[Header("Drawn Healthbar Settings ")]
		[PLInformation("Set the size (in world units), padding, back and front colors of the healthbar.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		/// the rotation to apply to the PLHealthBarContainer when drawing it
		[Tooltip("the rotation to apply to the PLHealthBarContainer when drawing it")]
		...
		public Gradient ForegroundColor = new Gradient()
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(PLColors.BestRed, 0),
				new GradientColorKey(PLColors.BestRed, 1f)
			},
		...
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(PLColors.Orange, 0),
				new GradientColorKey(PLColors.Orange, 1f)
			},
			alphaKeys = new GradientAlphaKey[2] { new GradientAlphaKey(1, 0), new GradientAlphaKey(1, 1) }
		};
		...
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(PLColors.AntiqueWhite, 0),
				new GradientColorKey(PLColors.AntiqueWhite, 1f)
			},
			alphaKeys = new GradientAlphaKey[2] { new GradientAlphaKey(1, 0), new GradientAlphaKey(1, 1) }
		};
		...
		{
			colorKeys = new GradientColorKey[2] {
				new GradientColorKey(PLColors.Black, 0),
				new GradientColorKey(PLColors.Black, 1f)
			},
			alphaKeys = new GradientAlphaKey[2] { new GradientAlphaKey(1, 0), new GradientAlphaKey(1, 1) }
		};
		...
		public PLFollowTarget.UpdateModes FollowTargetMode = PLFollowTarget.UpdateModes.LateUpdate;
		/// if this is true, the drawn health bar will be nested below the PLHealthBar
		[Tooltip("if this is true, the drawn health bar will be nested below the PLHealthBar")]
		public bool NestDrawnHealthBar = false;
		/// if this is true, a PLBillboard component will be added to the progress bar to make sure it always looks towards the camera
		[Tooltip("if this is true, a PLBillboard component will be added to the progress bar to make sure it always looks towards the camera")]
		...
		[Header("Offset")]
		[PLInformation("Set the offset (in world units), relative to the object's center, to which the health bar will be displayed.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		[Header("Display")]
		[PLInformation("Here you can define whether or not the healthbar should always be visible. If not, you can set here how long after a hit it'll remain visible.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		protected PLFollowTarget _followTransform;
		...
		public virtual void Initialization()
		{
			...
			switch (HealthBarType)
			{
				case HealthBarTypes.Prefab:
					...
					_progressBar = Instantiate(HealthBarPrefab, transform.position + HealthBarOffset, transform.rotation) as PLProgressBar;
		...
		protected virtual void DrawHealthBar()
		{
			...
			_progressBar = newGameObject.AddComponent<PLProgressBar>();

			_followTransform = newGameObject.AddComponent<PLFollowTarget>();
			...
			container.name = "PLProgressBarContainer";
			...
			if (Billboard)
			{
				_progressBar.gameObject.AddComponent<PLBillboard>();
			}
			...
			_progressBar.TimeScale = (TimeScale == TimeScales.Time) ? PLProgressBar.TimeScales.Time : PLProgressBar.TimeScales.UnscaledTime;

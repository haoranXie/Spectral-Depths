...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Camera/PLOrbitalCamera")]
	public class PLOrbitalCamera : MonoBehaviour
	{
		...
		public virtual void Initialization()
		{
			...
			if (Target == null)
			{
				Debug.LogError(this.gameObject.name + " : the PLOrbitalCamera doesn't have a target.");
				return;
			}
		...
		protected virtual void Rotation()
		{
			...
			if (Mode == Modes.Touch && (Input.touchCount > 0))
			{
				if ((Input.touches[0].phase == TouchPhase.Moved) && (Input.touchCount == 1))
				{
					...
					_angleY = PLMaths.ClampAngle(_angleY, MinVerticalAngleLimit, MaxVerticalAngleLimit);

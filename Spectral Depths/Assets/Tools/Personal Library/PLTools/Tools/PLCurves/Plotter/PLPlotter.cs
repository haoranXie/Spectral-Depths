...
{
	public class PLPlotter : MonoBehaviour
	{
		...
		protected virtual void FillMethodList()
		{
			...
			MethodInfo[] methods = typeof(PLTweenDefinitions).GetMethods(flags);
		...
		protected virtual void DrawPoints()
		{
			for (int i = 0; i < _points.Length; i++)
			{
				...
				_point.gameObject.PLGetComponentNoAlloc<MeshRenderer>().material = PlotPointMaterial;
		...
		{
			this.transform.PLDestroyAllChildren();
		}
		...
		protected virtual void Update()
		{
			...
			if (Time.unscaledTime - _lastMovementEndedAt < MovementPauseDuration)
			{
				...
				{
					_currentMovement = 1f;
					_newScaleUnit = PLTween.Tween(Time.unscaledTime - _lastMovementEndedAt, 0f, (MovementPauseDuration / 2f), 1f, 0f, PLTween.PLTweenCurve.EaseInCubic);
				...
				{
					_currentMovement = 0f;
					_newScaleUnit = PLTween.Tween(Time.unscaledTime - _lastMovementEndedAt, (MovementPauseDuration / 2f), MovementPauseDuration, 0f, 1f, PLTween.PLTweenCurve.EaseOutCubic);
					_newScale = Vector3.one * _newScaleUnit;

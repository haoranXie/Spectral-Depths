...
{	
	...
	/// This class used to be known as LoadingSceneManager, and has now been renamed to PLSceneLoadingManager for consistency
	/// </summary>
	public class PLSceneLoadingManager : MonoBehaviour 
	{
		...
		protected static PLTweenType _tween;
		...
		{
			_tween = new PLTweenType(PLTween.PLTweenCurve.EaseOutCubic);
		...
		{
			Time.timeScale = 1f;
			_progressBarImage.fillAmount = PLMaths.Approach(_progressBarImage.fillAmount,_fillTarget,Time.deltaTime*ProgressBarSpeed);
		}
		...
		protected virtual IEnumerator LoadAsynchronously() 
		{
			...
			// we fade from black
			PLFadeOutEvent.Trigger(StartFadeDuration, _tween);
			...
			// we fade to black
			PLFadeInEvent.Trigger(ExitFadeDuration, _tween);
		...
		protected virtual void LoadingComplete() 
		{
			...
			StartCoroutine(PLFade.FadeCanvasGroup(LoadingProgressBar,0.1f,0f));
			StartCoroutine(PLFade.FadeCanvasGroup(LoadingAnimation,0.1f,0f));
			StartCoroutine(PLFade.FadeCanvasGroup(LoadingCompleteAnimation,0.1f,1f));
		}
	}
}
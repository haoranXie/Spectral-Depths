...
{	
	...
	/// </summary>
	public abstract class PLObjectPooler : MonoBehaviour
	{
		/// singleton pattern
		public static PLObjectPooler Instance;
		...
		[PLCondition("NestWaitingPool", true)] 
		...
		protected PLObjectPool _objectPool;
		protected const int _initialPoolsListCapacity = 5;
		protected bool _onSceneLoadedRegistered = false;
        
		public static List<PLObjectPool> _pools = new List<PLObjectPool>(_initialPoolsListCapacity);

		/// <summary>
		/// Adds a pooler to the static list if needed
		/// </summary>
		/// <param name="pool"></param>
		public static void AddPool(PLObjectPool pool)
		{
			if (_pools == null)
			{
				_pools = new List<PLObjectPool>(_initialPoolsListCapacity);    
			}
		...
		/// </summary>
		/// <param name="pool"></param>
		public static void RemovePool(PLObjectPool pool)
		{
			_pools?.Remove(pool);
		}
		...
		protected virtual bool CreateWaitingPool()
		{
			if (!MutualizeWaitingPools)
			{
				...
				_objectPool = _waitingPool.AddComponent<PLObjectPool>();
			...
			{
				PLObjectPool objectPool = ExistingPool(DetermineObjectPoolName());
				if (objectPool != null)
				{
		...
		/// <returns></returns>
		public virtual PLObjectPool ExistingPool(string poolName)
		{
			if (_pools == null)
			{
				_pools = new List<PLObjectPool>(_initialPoolsListCapacity);    
			...
			if (_pools.Count == 0)
			{
				var pools = FindObjectsOfType<PLObjectPool>();
				if (pools.Length > 0)
				{
					_pools.AddRange(pools);
				}
			}
			foreach (PLObjectPool pool in _pools)
			{

...
namespace SpectralDepths.Tools
{
	[System.Serializable]
	/// <summary>
	/// This class describes a node on an PLPath
	/// </summary>
	public class PLPathMovementElement
	{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Movement/PLPath")]
	public class PLPath : MonoBehaviour 
	{
		...
		[Header("Path")]
		[PLInformation("Here you can select the '<b>Cycle Option</b>'. Back and Forth will have your object follow the path until its end, and go back to the original point. If you select Loop, the path will be closed and the object will move along it until told otherwise. If you select Only Once, the object will move along the path from the first to the last point, and remain there forever.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		public CycleOptions CycleOption;

		[PLInformation("Add points to the <b>Path</b> (set the size of the path first), then position the points using either the inspector or by moving the handles directly in scene view. For each path element you can specify a delay (in seconds). The order of the points will be the order the object follows.\nFor looping paths, you can then decide if the object will go through the points in the Path in Ascending (1, 2, 3...) or Descending (Last, Last-1, Last-2...) order.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		public List<PLPathMovementElement> PathElements;
		/// another PLPath that you can reference. If set, the reference PLPath's data will replace this PLPath's
		public PLPath ReferenceMMPath;
		...
		public virtual void Initialization()
		{
			...
			if (PathElements[0].PathElementPosition != Vector3.zero)
			{
				...
				foreach (PLPathMovementElement element in PathElements)
				{
		...
		{	
			...
			for (int i=0;i<PathElements.Count;i++)
			{
				...
				PLDebug.DrawGizmoPoint(_originalTransformPosition+PathElements[i].PathElementPosition,0.2f,Color.green);
			...
			if (Application.isPlaying)
			{
				...
				{
					PLDebug.DrawGizmoPoint(_originalTransformPosition + _currentPoint.Current,0.2f,Color.blue);
					PLDebug.DrawGizmoPoint(_originalTransformPosition + _previousPoint,0.2f,Color.red);	
				}
			}
			#endif
		...
		/// <summary>
		/// Replaces this PLPath's settings with the ones passed in parameters
		/// </summary>
		...
		public void SetPath(in Data configuration)
		{
			...
			PathElements = PathElements ?? new List<PLPathMovementElement>(configuration.Offsets.Length);
			PathElements.Clear();

			foreach (var offset in configuration.Offsets)
			{
				PathElements.Add(new PLPathMovementElement() {Delay = configuration.Delay, PathElementPosition = offset});
			}

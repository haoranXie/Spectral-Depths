...
namespace SpectralDepths.Tools
{
	...
	/// via the PLSoundManager
	/// </summary>
	public class PLSoundManagerTrackVolumeSlider :  MonoBehaviour, 
													...
													PLEventListener<PLSoundManagerTrackFadeEvent>
	{
		...
		protected virtual void LateUpdate()
		{
			if (Mode == Modes.Read)
			{
				float trackVolume = PLSoundManager.Instance.GetTrackVolume(Track, false);
		...
		public virtual void UpdateVolume(float newValue)
		{
			...
			float newVolume = PLMaths.Remap(newValue, 0f, 1f, MinVolume, MaxVolume);
			PLSoundManagerTrackEvent.Trigger(PLSoundManagerTrackEventTypes.SetVolumeTrack, Track, newVolume);
		}
		...
		public void OnMMEvent(PLSoundManagerEvent soundManagerEvent)
		{
			if (soundManagerEvent.EventType == PLSoundManagerEventTypes.SettingsLoaded)
			{
		...
		{
			_slider.value = PLMaths.Remap(PLSoundManager.Instance.GetTrackVolume(Track, false), 0f, 1f, MinVolume, MaxVolume);
		}
		...
		public void OnMMEvent(PLSoundManagerTrackEvent trackEvent)
		{
		...
		public void OnMMEvent(PLSoundManagerTrackFadeEvent fadeEvent)
		{

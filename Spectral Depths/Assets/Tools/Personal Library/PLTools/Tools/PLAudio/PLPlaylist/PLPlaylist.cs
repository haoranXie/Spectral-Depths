...
namespace SpectralDepths.Tools
{
	...
	public struct PLPlaylistPlayEvent
	{
	...
	public struct PLPlaylistStopEvent
	{
		static private event Delegate OnEvent;
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)] private static void RuntimeInitialization() { OnEvent = null; }
		static public void Register(Delegate callback) { OnEvent += callback; }
	...
	public struct PLPlaylistPauseEvent
	{
		static private event Delegate OnEvent;
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)] private static void RuntimeInitialization() { OnEvent = null; }
		static public void Register(Delegate callback) { OnEvent += callback; }
	...
	public struct PLPlaylistPlayNextEvent
	{
		static private event Delegate OnEvent;
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)] private static void RuntimeInitialization() { OnEvent = null; }
		static public void Register(Delegate callback) { OnEvent += callback; }
	...
	public struct PLPlaylistPlayPreviousEvent
	{
		static private event Delegate OnEvent;
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)] private static void RuntimeInitialization() { OnEvent = null; }
		static public void Register(Delegate callback) { OnEvent += callback; }
	...
	public struct PLPlaylistPlayIndexEvent
	{
		static private event Delegate OnEvent;
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)] private static void RuntimeInitialization() { OnEvent = null; }
		static public void Register(Delegate callback) { OnEvent += callback; }
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Audio/PLPlaylist")]
	[PLRequiresConstantRepaint]
	public class PLPlaylist : PLMonoBehaviour
	{
		...
		[PLInspectorGroup("Playlist Songs", true, 18)]
		...
		public List<PLPlaylistSong> Songs;

		[PLInspectorGroup("Settings", true, 13)]
		...
		[PLInspectorGroup("Persistence", true, 32)]
		...
		[PLCondition("Persistent", true)]
		public bool AutomaticallyUnparentOnAwake = true;

		[PLInspectorGroup("Status", true, 14)]
		...
		[PLReadOnly]
		public PLStateMachine<PLPlaylist.PlaylistStates> PlaylistState;

		[PLInspectorGroup("Tests", true, 15)]
		...
		[PLInspectorButton("PlayNextSong")]
		...
		/// a next song test button
		[PLInspectorButton("PlayTargetSong")]
		...
		public static PLPlaylist Current => _instance;
		protected static PLPlaylist _instance;
		...
		public static PLPlaylist Instance
		{
			get
			{
				if (_instance == null)
				{
					_instance = FindObjectOfType<PLPlaylist> ();
					if (_instance == null)
					{
						GameObject obj = new GameObject ();
						obj.name = typeof(PLPlaylist).Name + "_AutoCreated";
						_instance = obj.AddComponent<PLPlaylist> ();
					}
				}
				return _instance;
			}
		...
		protected virtual void Initialization()
		{
			...
			_songsPlayedSoFar = 0;
			PlaylistState = new PLStateMachine<PLPlaylist.PlaylistStates>(this.gameObject, true);
		...
		protected virtual IEnumerator PlaySong(int index)
		{
			...
			{
				foreach (PLPlaylistSong song in Songs)
				{
			...
			// initial delay
			yield return PLCoroutine.WaitFor(Random.Range(Songs[index].InitialDelay.x, Songs[index].InitialDelay.y));
		...
		protected virtual IEnumerator Fade(int index, float duration, float initialVolume, float endVolume, bool stopAtTheEnd)
		{
			...
			{
				progress = PLMaths.Remap(Time.time - startTimestamp, 0f, duration, 0f, 1f);
		...
		{
			PLPlaylistPauseEvent.Register(OnPauseEvent);
			PLPlaylistPlayEvent.Register(OnPlayEvent);
			PLPlaylistPlayNextEvent.Register(OnPlayNextEvent);
			PLPlaylistPlayPreviousEvent.Register(OnPlayPreviousEvent);
			PLPlaylistStopEvent.Register(OnStopEvent);
			PLPlaylistPlayIndexEvent.Register(OnPlayIndexEvent);
		}
		...
		{
			PLPlaylistPauseEvent.Unregister(OnPauseEvent);
			PLPlaylistPlayEvent.Unregister(OnPlayEvent);
			PLPlaylistPlayNextEvent.Unregister(OnPlayNextEvent);
			PLPlaylistPlayPreviousEvent.Unregister(OnPlayPreviousEvent);
			PLPlaylistStopEvent.Unregister(OnStopEvent);
			PLPlaylistPlayIndexEvent.Unregister(OnPlayIndexEvent);
		}

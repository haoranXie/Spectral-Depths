...
{
	[System.Serializable]
	public class PLRadioSignalOnValueChange : UnityEvent<float> { }

	/// <summary>
	/// A class used to define a signal, meant to be broadcasted by a PLRadioBroadcaster
	/// It'll output a Level value to broadcast, using one time, persistent or driven modes    
	/// Meant to be extended
	/// </summary>
	public abstract class PLRadioSignal : MonoBehaviour
	{
		...
		/// the level, to read from a PLRadioBroadcaster
		...
		public PLRadioSignalOnValueChange OnValueChange;
		...
		protected virtual void Update()
		{
			...
			else if (SignalMode == SignalModes.Persistent)
			{
				...
				DriverTime = PLMaths.Remap(_signalTime, 0f, Duration, 0f, 1f);
			}
		...
		public virtual float ApplyBias(float t, float bias)
		{
			...
			bias = PLMaths.Remap(bias, 0f, 1f, 1f, 0f);

...
namespace SpectralDepths.Tools
{
	...
	/// You can then use these values from a PLRadioBroadcaster, or simply evaluate its value to use wherever you want, 
	/// like a supercharged animation curve. In that case, simply disable the component, and read from it using its Evaluate method
	/// </summary>
	public class PLRadioSignalGenerator : PLRadioSignal
	{
		...
		void Reset()
		{
			SignalList = new PLRadioSignalGeneratorItemList(){
				new PLRadioSignalGeneratorItem()
			};
		}
		...
		public virtual float Evaluate(float time)
		{
			...
			for (int i = 0; i < SignalList.Count; i++)
			{
				...
				{
					float newLevel = PLSignal.GetValueNormalized(time,
					...
					level = (SignalList[i].Mode == PLRadioSignalGeneratorItem.GeneratorItemModes.Multiply) ? level * newLevel : level + newLevel;
		...
		protected override void Shake()
		{
			...
			if (SignalMode == SignalModes.OneTime)
			{
				...
				CurrentLevel = Evaluate(PLMaths.Remap(elapsedTime, 0f, Duration, 0f, 1f));   
			}
			else
			{
	...
	/// </summary>
	[System.Serializable]
	public class PLRadioSignalGeneratorItem
	{
		...
		public PLSignal.SignalType SignalType = PLSignal.SignalType.Sine;
		/// if the type is animation curve, the curve to consider
		[PLEnumCondition("SignalType", (int)PLSignal.SignalType.AnimationCurve)]
		public AnimationCurve Curve = new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1));
		/// if the type is PLTween, the tween to consider
		[PLEnumCondition("SignalType", (int)PLSignal.SignalType.PLTween)]
		public PLTween.PLTweenCurve TweenCurve = PLTween.PLTweenCurve.EaseInOutQuartic;
	...
	/// </summary>
	[System.Serializable]
	public class PLRadioSignalGeneratorItemList : PLReorderableArray<PLRadioSignalGeneratorItem>
	{
	}
}
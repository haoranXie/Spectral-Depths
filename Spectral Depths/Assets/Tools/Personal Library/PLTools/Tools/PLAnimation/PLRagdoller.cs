...
namespace SpectralDepths.Tools
{
	...
	/// If you have parts of your ragdoll that you don't want to be affected by this script (a weapon for example), just add a PLRagdollerIgnore component to them
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Animation/PLRagdoller")]
	public class PLRagdoller : MonoBehaviour
	{
		/// <summary>
			...
			set
			{
				if (value == true)
				{
					...
					if (CurrentState == RagdollStates.Animated)
					{
						...
						PLAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBackAnimationParameter, false, _animatorParameters);
						PLAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBellyAnimationParameter, false, _animatorParameters);
					}
				}
				else
				{
					if (CurrentState == RagdollStates.Ragdolling)
					{
						...
						{
							PLAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBackAnimationParameter, true, _animatorParameters);
						}
						else
						{
							PLAnimatorExtensions.UpdateAnimatorBool(_animator, _getUpFromBellyAnimationParameter, true, _animatorParameters);
						}
					}
				}
			}
		...
		protected virtual void Initialization()
		{
			...
			{
				if (rigidbody.gameObject.PLGetComponentNoAlloc<PLRagdollerIgnore>() == null)
				{
		...
		protected virtual void RegisterAnimatorParameters()
		{
			...
			if (_animator.PLHasParameterOfType(_getUpFromBackAnimationParameterName, AnimatorControllerParameterType.Bool))
			{
				_animatorParameters.Add(_getUpFromBackAnimationParameter);
			}
			if (_animator.PLHasParameterOfType(_getUpFromBellyAnimationParameterName, AnimatorControllerParameterType.Bool))
			{

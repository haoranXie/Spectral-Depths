...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Activation/PLTimedActivation")]
	public class PLTimedActivation : MonoBehaviour
	{
		...
		[PLEnumCondition("ActivationMode", (int)ActivationModes.OnTriggerEnter, (int)ActivationModes.OnTriggerExit)]
		public TriggerModes TriggerMode;
		/// the layer the target collider should be on
		[PLEnumCondition("TriggerMode", (int)TriggerModes.Layer)]
		public LayerMask TargetTriggerLayer;
		/// the tag the target collider should have
		[PLEnumCondition("TriggerMode", (int)TriggerModes.Tag)]
		...
		protected virtual IEnumerator TimedActivationSequence()
		{
			...
			{
				yield return PLCoroutine.WaitFor(TimeBeforeStateChange);
			}
			else
			{
				yield return StartCoroutine(PLCoroutine.WaitForFrames(FrameCount));
			}
			StateChange();
			Activate();
		}

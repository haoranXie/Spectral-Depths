...
namespace SpectralDepths.Tools
{
	...
	[AddComponentMenu("Spectral Depths/Tools/Property Controllers/ShaderController")]
	public class ShaderController : PLMonoBehaviour
	{
		...
		[PLEnumCondition("TargetType",(int)TargetTypes.Renderer)]
		...
		[PLEnumCondition("TargetType", (int)TargetTypes.Image)]
		...
		[PLEnumCondition("PropertyType", (int)PropertyTypes.Vector)]
		public bool X;
		...
		[PLCondition("UseMaterialPropertyBlocks", true)]
		...
		[PLCondition("RemapNoiseValues", true)]
		...
		public PLAudioAnalyzer AudioAnalyzer;
		...
		/// a test button for the one time shake
		[Tooltip("a test button for the one time shake")]
		[PLInspectorButton("ToDestination")]
		...
		{
			DrivenLevel = PLMaths.Remap(normalizedLevel, 0f, 1f, remapZero, remapOne);
		}
		...
		protected virtual void UpdateValue()
		{
			...
			switch (ControlMode)
			{
				case ControlModes.PingPong:
					...
					CurrentValue = PLTween.Tween(PingPong, 0f, Duration, MinValue, MaxValue, Curve);
					CurrentValueNormalized = PLMaths.Remap(CurrentValue, MinValue, MaxValue, 0f, 1f);
					break;
				case ControlModes.Loop:
					...
					CurrentValue = PLTween.Tween(LoopTime, 0f, LoopDuration, LoopStartValue, LoopEndValue, LoopCurve);
					CurrentValueNormalized = PLMaths.Remap(CurrentValue, LoopStartValue, LoopEndValue, 0f, 1f);
					break;
				case ControlModes.Random:
					...
					if (RemapNoiseValues)
					{
						...
						CurrentValue = PLMaths.Remap(CurrentValue, 0f, 1f, RemapNoiseZero, RemapNoiseOne);
					}
					else
					{
				...
				case ControlModes.OneTime:
					...
					_remappedTimeSinceStart = PLMaths.Remap(GetTime() - _startedTimestamp, 0f, OneTimeDuration, 0f, 1f);
					CurrentValueNormalized = OneTimeCurve.Evaluate(_remappedTimeSinceStart);
					CurrentValue = PLMaths.Remap(CurrentValueNormalized, 0f, 1f, OneTimeRemapMin, OneTimeRemapMax);
				...
				case ControlModes.ToDestination:
					...
					_remappedTimeSinceStart = PLMaths.Remap(GetTime() - _startedTimestamp, 0f, ToDestinationDuration, 0f, 1f);
					...
					CurrentValueNormalized = PLMaths.Remap(CurrentValue, _initialValue, ToDestinationValue, 0f, 1f);
					break;
			}
			...
			if ((ControlMode == ControlModes.OneTime) && _shaking && (GetTime() - _startedTimestamp > OneTimeDuration))
			{
				...
				else
				{
					...
					CurrentValue = PLMaths.Remap(CurrentValue, 0f, 1f, OneTimeRemapMin, OneTimeRemapMax);
					CurrentValue *= OneTimeAmplitude;
				}
				SetValue(CurrentValue);

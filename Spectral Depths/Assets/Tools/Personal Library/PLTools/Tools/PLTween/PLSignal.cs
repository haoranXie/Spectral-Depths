...
namespace SpectralDepths.Tools
{
	...
	public class PLSignal
	{
		...
		/// <returns></returns>
		public static float GetValue(float time, SignalType signalType, float phase, float amplitude, float frequency, float offset, bool Invert = false, AnimationCurve curve = null, PLTween.PLTweenCurve tweenCurve = PLTween.PLTweenCurve.LinearTween)
		{
			float value = 0f;
			...
			{ 
				...
				case SignalType.AnimationCurve:
					...
					break;
				case SignalType.PLTween:
					t = (t != 1f) ? t - Mathf.Floor(t) : 1f;
					value = PLTween.Tween(t, 0f, 1f, 0f, 1f, tweenCurve);
					break;
			}
		...
		public static float GetValueNormalized(float time, SignalType signalType,
			...
			AnimationCurve curve = null, PLTween.PLTweenCurve tweenCurve = PLTween.PLTweenCurve.LinearTween,
			bool clamp = true, float clampMin = 0f, float clampMax = 1f, bool backAndForth = false, float backAndForthTippingPoint = 0.5f)
		{
			...
			if (backAndForth)
			{
				...
				{
					time = PLMaths.Remap(time, 0f, backAndForthTippingPoint, 0f, 1f);
				}
				...
				{
					time = PLMaths.Remap(time, backAndForthTippingPoint, 1f, 1f, 0f);
				}
			}
			...
			switch (signalType)
			{
				case SignalType.Sine:
					value = (float)Mathf.Sin(2f * Mathf.PI * t);
					value = PLMaths.Remap(value, -1f, 1f, 0f, 1f);
					break;
				case SignalType.Square:
					value = Mathf.Sign(Mathf.Sin(2f * Mathf.PI * t));
					value = PLMaths.Remap(value, -1f, 1f, 0f, 1f);
			...
			if (Invert)
			{
				value = PLMaths.Remap(value, 0f, 1f, 1f, 0f);
			}

...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	[CustomEditor(typeof(PLFeedbacks))]
	public class PLFeedbacksEditor : Editor
	{
		/// <summary>
		...
		static class FeedbackCopy
		{
			...
			static public void CopyAll(PLFeedbacks sourceFeedbacks)
			{
				PLFeedbacksConfiguration.Instance._mmFeedbacks = sourceFeedbacks;
			}

			static public bool HasMultipleCopies()
			{
				return (PLFeedbacksConfiguration.Instance._mmFeedbacks != null);
			}

			static public void PasteAll(PLFeedbacksEditor targetEditor)
			{
				var sourceFeedbacks = new SerializedObject(PLFeedbacksConfiguration.Instance._mmFeedbacks);
				SerializedProperty feedbacks = sourceFeedbacks.FindProperty("Feedbacks");

				for (int i = 0; i < feedbacks.arraySize; i++)
				{
					PLFeedback arrayFeedback = (feedbacks.GetArrayElementAtIndex(i).objectReferenceValue as PLFeedback);

					FeedbackCopy.Copy(new SerializedObject(arrayFeedback));
					PLFeedback newFeedback = targetEditor.AddFeedback(arrayFeedback.GetType());
					SerializedObject serialized = new SerializedObject(newFeedback);
					serialized.Update();
					FeedbackCopy.Paste(serialized);
					serialized.ApplyModifiedProperties();
				}
				PLFeedbacksConfiguration.Instance._mmFeedbacks = null;
			}
		}

		protected PLFeedbacks _targetMMFeedbacks;
		...
		protected Dictionary<PLFeedback, Editor> _editors;
		...
		{
			// Get properties
			_targetMMFeedbacks = target as PLFeedbacks;
			...
			// Create editors
			_editors = new Dictionary<PLFeedback, Editor>();
			for (int i = 0; i < _mmfeedbacks.arraySize; i++)
			{
				AddEditor(_mmfeedbacks.GetArrayElementAtIndex(i).objectReferenceValue as PLFeedback);
			}
		...
		{
			_targetMMFeedbacks = target as PLFeedbacks;
			if ((_targetMMFeedbacks.SafeMode == PLFeedbacks.SafeModes.EditorOnly) || (_targetMMFeedbacks.SafeMode == PLFeedbacks.SafeModes.Full))
			{
		...
		public override void OnInspectorGUI()
		{
			...
			if (!PLFeedbacks.GlobalMMFeedbacksActive)
			{
				Color baseColor = GUI.color;
				GUI.color = Color.red;
				EditorGUILayout.HelpBox("All PLFeedbacks, including this one, are currently disabled. This is done via script, by changing the value of the PLFeedbacks.GlobalMMFeedbacksActive boolean. Right now this value has been set to false. Setting it back to true will allow PLFeedbacks to play again.", MessageType.Warning);
				EditorGUILayout.Space();
				GUI.color = baseColor;
			}

			if (PLFeedbacksConfiguration.Instance.ShowInspectorTips)
			{
				EditorGUILayout.HelpBox("The PLFeedbacks component is getting replaced by the new and improved PLF_Player, which will improve performance, let you keep runtime changes, and much more! The PLF_Player works just like PLFeedbacks. " +
			...
			if (_settingsMenuDropdown)
			{
				...
				if (!Application.isPlaying)
				{
					...
					if (GUILayout.Button("Generate PLF_Player"))
					{
						this.ConvertToMMF_Player(true);
					}
					if (GUILayout.Button("Convert to PLF_Player"))
					{
			...
			if ((target as PLFeedbacks).IsPlaying)
			{
			...
			if (_targetMMFeedbacks.Direction == PLFeedbacks.Directions.BottomToTop)
			{
			...
			PLFeedbackStyling.DrawSection("Feedbacks");
			...
			for (int i = 0; i < _mmfeedbacks.arraySize; i++)
			{
				...
				PLFeedbackStyling.DrawSplitter();
				...
				PLFeedback feedback = property.objectReferenceValue as PLFeedback;
				...
				{
					if ((feedback as PLFeedbackLooper).InfiniteLoop)
					{
						label = label + "[Infinite Loop] ";
					}
					else
					{
						label = label + "[ " + (feedback as PLFeedbackLooper).NumberOfLoopsLeft + " loops left ] ";
					}                    
				}

				Rect headerRect = PLFeedbackStyling.DrawHeader(
					ref isExpanded,
					ref feedback.Active,
					label,
				...
				if (isExpanded)
				{
					...
					if ( (!string.IsNullOrEmpty(helpText)) && (PLFeedbacksConfiguration.Instance.ShowInspectorTips))
					{
			...
			if (wasRemoved)
			{
				GameObject gameObject = (target as PLFeedbacks).gameObject;
			...
			PLFeedbackStyling.DrawSection("All Feedbacks Debug");
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				{
					(target as PLFeedbacks).Initialization();
				}
				...
				if (GUILayout.Button("Play", EditorStyles.miniButtonMid))
				{
					(target as PLFeedbacks).PlayFeedbacks();
				}
				GUI.backgroundColor = _originalBackgroundColor;
                
				// pause button
				if ((target as PLFeedbacks).ContainsLoop)
				{
					if (GUILayout.Button("Pause", EditorStyles.miniButtonMid))
					{
						(target as PLFeedbacks).PauseFeedbacks();
					}   
				}
				...
				if (GUILayout.Button("Stop", EditorStyles.miniButtonMid))
				{
					(target as PLFeedbacks).StopFeedbacks();
				}
                
				// reset button
				if (GUILayout.Button("Reset", EditorStyles.miniButtonMid))
				{
					(target as PLFeedbacks).ResetFeedbacks();
				}
				EditorGUI.EndDisabledGroup();
				...
				EditorGUI.BeginChangeCheck();
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						foreach (var f in (target as PLFeedbacks).Feedbacks)
		...
		/// </summary>
		protected virtual PLFeedback AddFeedback(System.Type type)
		{
			/*GameObject gameObject = (target as PLFeedbacks).gameObject;

			PLFeedback newFeedback = Undo.AddComponent(gameObject, type) as PLFeedback;
			...
			return newFeedback;*/
			return (target as PLFeedbacks).AddFeedback(type);
		}
		...
		protected virtual void RemoveFeedback(int id)
		{
			...
			(target as PLFeedbacks).Feedbacks.Remove(feedback);

			_editors.Remove(feedback);
			Undo.DestroyObjectImmediate(feedback);*/
            
			(target as PLFeedbacks).RemoveFeedback(id);
		}
		...
		/// </summary>
		protected virtual void AddEditor(PLFeedback feedback)
		{
			if (feedback == null)
				return;
		...
		/// </summary>
		protected virtual void RemoveEditor(PLFeedback feedback)
		{
			if (feedback == null)
				return;
		...
		{
			FeedbackCopy.CopyAll(target as PLFeedbacks);
		}
		...
		{
			PLFeedback newFeedback = AddFeedback(FeedbackCopy.Type);
			SerializedObject serialized = new SerializedObject(newFeedback);
		...
		{
			serializedObject.Update();
			Undo.RecordObject(target, "Paste all PLFeedbacks");
			FeedbackCopy.PasteAll(this);
			serializedObject.ApplyModifiedProperties();
		}

		/// <summary>
		/// Converts a PLFeedbacks and all its contents into the new and improved PLF_Player
		/// To convert from PLFeedbacks to PLF_Player you have a few options :
		/// - on a PLFeedbacks Settings panel, press the Generate PLF_Player button, this will create a new PLF_Player component on the same object, and copy feedbacks to it, ready to use.
		///     The old PLFeedbacks won't be touched.
		/// - press the Convert feedback, and this will try to replace the current PLFeedbacks with a new PLF_Player with the same settings.
		/// - if you're inside a prefab instance, regular replace won't work, and you'll want to add, at the prefab level, an empty PLF_Player that will be the recipient of the conversion
		/// 
		/// </summary>
		public virtual void ConvertToMMF_Player(bool generateOnly)
		{
			GameObject targetObject = _targetMMFeedbacks.gameObject;
			PLF_Player oldMMFPlayer = targetObject.GetComponent<PLF_Player>();
            
			// we remove any PLF_Player that may already be on that object
			...
			serializedObject.Update();
			Undo.RegisterCompleteObjectUndo(target, "Convert to PLF_Player");
			Debug.Log("Starting conversion to PLF_Player --------");
			...
			if (generateOnly)
			{
				...
				{
					PLF_Player newPlayer = targetObject.AddComponent<PLF_Player>();
			...
			if (oldMMFPlayer == null)
			{
				...
				PLF_Player newPlayer = temporaryHost.AddComponent<PLF_Player>();
				CopyFromMMFeedbacksToMMF_Player(newPlayer);
				...
				PLF_Player finalPlayer = targetObject.GetComponent<PLF_Player>();
				...
				{
					foreach (PLF_Feedback feedback in finalPlayer.FeedbacksList)
					{
			...
			Component[] feedbackArray = targetObject.GetComponents(typeof(PLFeedback));
		...
		protected virtual void CopyFromMMFeedbacksToMMF_Player(PLF_Player newPlayer)
		{
			...
			for (int i = 0; i < feedbacks.arraySize; i++)
			{
				PLFeedback oldFeedback = (feedbacks.GetArrayElementAtIndex(i).objectReferenceValue as PLFeedback);
                
				// we look for a match in the new classes
				Type oldType = oldFeedback.GetType();
				string oldTypeName = oldType.Name.ToString();
				string newTypeName = oldTypeName.Replace("PLFeedback", "PLF_");
				Type newType = PLFeedbackStaticMethods.PLFGetTypeByName(newTypeName);
                
				if (newType == null)
				{
					Debug.Log("<color=red>Couldn't find any PLF_Feedback matching "+oldTypeName+"</color>");
				}
				else
				{
					PLF_Feedback newFeedback = newPlayer.AddFeedback(newType);
                    
					List<FieldInfo> oldFieldsList;
					int oldFieldsListLength = PLF_FieldInfo.GetFieldInfo(oldFeedback, out oldFieldsList);

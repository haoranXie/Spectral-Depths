...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	[CustomEditor(typeof(PLF_Player), true)]
	public class PLF_PlayerEditor : Editor
	{
		/// <summary>
		...
		public PLF_Player TargetMmfPlayer;
		...
		protected PLF_Feedback _feedbackListFeedback;
		protected Dictionary<PLF_Feedback, Editor> _editors;
		...
		protected Dictionary<int, PLF_FeedbackInspector> PLF_FeedbackInspectors =
			new Dictionary<int, PLF_FeedbackInspector>();
		...
		protected const string _inactiveMessage = "All PLFeedbacks, including this one, are currently disabled. This is done via script, by changing the value of the PLFeedbacks.GlobalMMFeedbacksActive boolean. Right now this value has been set to false. Setting it back to true will allow PLFeedbacks to play again.";
		...
		{
			// Get properties
			TargetMmfPlayer = target as PLF_Player;
			...
			_expandGroupsInInspectors = PLF_PlayerConfiguration.Instance.InspectorGroupsExpandedByDefault;
			...
			foreach (KeyValuePair<int, PLF_FeedbackInspector> inspector in PLF_FeedbackInspectors)
			{
		...
		protected virtual void PrepareFeedbackTypeList()
		{
			...
			for (int i = 0; i < types.Count; i++)
			{
				...
				if ((_newType.FeedbackName == "PLF_FeedbackBase") || (_newType.FeedbackName == null))
				{
					continue;
				}
		...
		protected virtual void InspectorCaching()
		{
			...
			PLF_PlayerStyling.CacheStyling();
		...
		protected virtual void DrawInspectorActiveWarning()
		{
			...
			if (!PLF_Player.GlobalMMFeedbacksActive)
			{
		...
		{
			if (PLF_PlayerConfiguration.Instance.ShowInspectorTips)
			{
		...
		protected virtual void DrawDurationAndDirection()
		{
			...
			if ((target as PLF_Player).IsPlaying)
			{
			...
			if (TargetMmfPlayer.Direction == PLF_Player.Directions.BottomToTop)
			{
		...
		{
			PLF_PlayerStyling.DrawSection(_allFeedbacksDebugText);
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				{
					(target as PLF_Player).Initialization();
				}
				...
				{
					(target as PLF_Player).PlayFeedbacks();
				}
				GUI.backgroundColor = _originalBackgroundColor;
                
				// pause button
				if ((target as PLF_Player).ContainsLoop)
				{
					if (GUILayout.Button(_pauseText, EditorStyles.miniButtonMid))
					{
						(target as PLF_Player).PauseFeedbacks();
					}   
				}
		...
		protected virtual void DrawBottomBar()
		{
			...
			{
				PLF_PlayerStyling.DrawSplitter();
			}
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				if (PLF_PlayerCopy.HasMultipleCopies())
				{
		...
		protected PLF_FeedbackInspector _mmfFeedbackInspector;

		protected virtual void DrawFeedbacksList()
		{
			PLF_PlayerStyling.DrawSection(_feedbacksSectionTitle);
			for (int i = 0; i < _mmfeedbacksList.arraySize; i++)
			{
				...
				if (_feedbackListIsExpanded)
				{
					...
					{
						if(PLF_FeedbackInspectors.TryGetValue(_feedbackListFeedback.UniqueID, out _mmfFeedbackInspector))
						{
							_mmfFeedbackInspector.DrawInspector(currentProperty, _feedbackListFeedback);
						}
						else
						{
							PLF_FeedbackInspector newInspector = new PLF_FeedbackInspector(); 
							PLF_FeedbackInspectors.Add(_feedbackListFeedback.UniqueID, newInspector);
		...
		protected virtual void DrawFeedbackHeader(int i)
		{
			...
			{
				if ((_feedbackListFeedback as PLF_Looper).InfiniteLoop)
				{
					_feedbackListLabel = _feedbackListLabel + _infiniteLoopText;
				}
				else
				{
					_feedbackListLabel = _feedbackListLabel + "[ " + (_feedbackListFeedback as PLF_Looper).NumberOfLoopsLeft + " loops left ] ";
				}                  
			}

			Rect headerRect = PLF_PlayerStyling.DrawHeader(
		...
		protected virtual void DrawFeedbackHelp()
		{
			...
			if ( (!string.IsNullOrEmpty(helpText)) && (PLF_PlayerConfiguration.Instance.ShowInspectorTips))
			{
		...
		/// </summary>
		protected virtual PLF_Feedback AddFeedback(System.Type type)
		{
			return (target as PLF_Player).AddFeedback(type);
		}
		...
		protected virtual void RemoveFeedback(int id)
		{
			...
			PLF_FeedbackInspectors.Remove(TargetMmfPlayer.FeedbacksList[id].UniqueID);
			(target as PLF_Player).RemoveFeedback(id);
			serializedObject.ApplyModifiedProperties();
			ForceRepaint();
			PrefabUtility.RecordPrefabInstancePropertyModifications(TargetMmfPlayer);
		}
		...
		protected virtual void ResetContextMenuFeedback(int id)
		{
			...
			Type feedbackType = (target as PLF_Player).FeedbacksList[id].GetType();
			PLF_Feedback newFeedback = (target as PLF_Player).AddFeedback(feedbackType, false);
			(target as PLF_Player).FeedbacksList[id] = newFeedback;
			serializedObject.ApplyModifiedProperties();
			ForceRepaint();
			PrefabUtility.RecordPrefabInstancePropertyModifications(TargetMmfPlayer);
		}
		...
		{
			PLF_Feedback feedback = TargetMmfPlayer.FeedbacksList[id];
		...
		{
			PLF_PlayerCopy.CopyAll(target as PLF_Player);
		}
		...
		public void ModeChanged(PlayModeStateChange playModeState)
		{
			switch (playModeState)
			{
				case PlayModeStateChange.ExitingPlayMode:
					if (_keepPlayModeChanges.boolValue)
					{
						PLF_PlayerCopy.ShouldKeepChanges.Add(TargetMmfPlayer);
						CopyAll();    
					}
					break;
				...
				case PlayModeStateChange.EnteredEditMode:
					...
					if (PLF_PlayerCopy.ShouldKeepChanges.Contains(TargetMmfPlayer))
					{
						ReplaceAll();
						if (PLF_PlayerConfiguration.Instance.AutoDisableKeepPlaymodeChanges)
						{
							serializedObject.Update();
							_keepPlayModeChanges.boolValue = false;    
							serializedObject.ApplyModifiedProperties();
						}
						PLF_PlayerCopy.ShouldKeepChanges.Remove(TargetMmfPlayer);
					}
					ForceRepaint();
		...
		public void ForceRepaint()
		{
			PLF_FeedbackInspectors.Clear();
			Initialization();
			(target as PLF_Player).RefreshCache();
			Repaint();
		}

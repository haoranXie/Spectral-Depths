...
{
	/// <summary>
	/// A class used to regroup most of the styling options for the PLFeedback editors
	/// </summary>
	public static class PLF_PlayerStyling
	{
		...
		static public Rect DrawHeader(ref bool expanded, ref bool activeField, string title, Color feedbackColor, System.Action<GenericMenu> fillGenericMenu, 
			float startedAt, float duration, float totalDuration, PLFeedbackTiming timing, bool pause, bool requiresSetup, string requiredTarget, Color displayColor, 
			bool displayFullHeaderColor, PLF_Player host)
		{
			...
			if (timing.PLFeedbacksDirectionCondition == PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenBackwards)
			{
                
				EditorGUI.LabelField(_directionRect, _directionUpIcon);
			}

			if (timing.PLFeedbacksDirectionCondition == PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenForwards)
			{

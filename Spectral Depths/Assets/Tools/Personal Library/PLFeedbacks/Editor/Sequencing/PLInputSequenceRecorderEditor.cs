...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	[CustomEditor(typeof(PLInputSequenceRecorder), true)]
	[CanEditMultipleObjects]
	public class PLInputSequenceRecorderEditor : Editor
	{
		...
		protected PLInputSequenceRecorder _targetRecorder;
		...
		protected virtual void OnEnable()
		{
			...
			_targetRecorder = (PLInputSequenceRecorder)target;
		...
		protected virtual void EditorDetectRecording()
		{
			if (_targetRecorder.Recording && (_targetRecorder.SequenceScriptableObject != null))
			{
				if (_currentEvent.isKey)
				{
					foreach (PLSequenceTrack track in _targetRecorder.SequenceScriptableObject.SequenceTracks)
					{
						if (_currentEvent.keyCode == (track.Key))
						{
							if (track.State == PLSequenceTrackStates.Up)
							{
								track.State = PLSequenceTrackStates.Idle;
							}
							if (_currentEvent.type == EventType.KeyDown)
							{
								if (track.State != PLSequenceTrackStates.Down)
								{
									// key is down for the first time
									_targetRecorder.AddNoteToTrack(track);
								}
								track.State = PLSequenceTrackStates.Down;
							}
							if (_currentEvent.type == EventType.KeyUp)
							{
								// key is up
								track.State = PLSequenceTrackStates.Up;
							}
						}
					}
				}

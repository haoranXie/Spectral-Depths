...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	public static class PLFDependencyInstaller 
	{
		#if UNITY_EDITOR
		...
		/// </summary>
		[MenuItem("Tools/Spectral Depths/PLFeedbacks/Install All Dependencies", false, 703)]
		public static void InstallAllDependencies()
		{
			...
			Debug.Log("[PLFDependencyInstaller] Installation start");
		...
		static void InstallNext()
		{
			if (_currentIndex < _packages.Length)
			{
				...
				foreach (var package in _listRequest.Result)
				{
					...
					{
						packageFound = true;
						Debug.Log("[PLFDependencyInstaller] "+package.name+" is already installed");
						_currentIndex++;
						InstallNext();
						return;
					} 
				...
				if (!packageFound)
				{
					Debug.Log("[PLFDependencyInstaller] installing "+_packages[_currentIndex]);
			...
			{
				Debug.Log("[PLFDependencyInstaller] Installation complete");
				Debug.Log("[PLFDependencyInstaller] It's recommended to now close that scene and reopen it before playing it.");
			}
		}
		...
		static void AddProgress()
		{
			if (_addRequest.IsCompleted)
			{
				...
				{
					Debug.Log("[PLFDependencyInstaller] "+_addRequest.Result.packageId+" has been installed");
					_currentIndex++;
					InstallNext();
				}

...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Fog : PLF_Feedback
	{
		...
		[PLFEnumCondition("Mode", (int)Modes.OverTime)]
		public float Duration = 2f;
		/// if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over
		[Tooltip("if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over")] 
		public bool AllowAdditivePlays = false;

		[PLFInspectorGroup("Fog Density", true, 25)]
		...
		public PLTweenType DensityCurve = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[PLFInspectorGroup("Fog Start Distance", true, 26)]
		...
		public PLTweenType StartDistanceCurve = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[PLFInspectorGroup("Fog End Distance", true, 27)]
		...
		public PLTweenType EndDistanceCurve = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[PLFInspectorGroup("Fog Color", true, 28)]
		...
		protected virtual void SetFogValues(float time, float intensityMultiplier)
		{
			...
			if (ModifyFogDensity)
			{
				RenderSettings.fogDensity = PLTween.Tween(time, 0f, 1f, DensityRemapZero, DensityRemapOne, DensityCurve) * intensityMultiplier;
			}

			if (ModifyStartDistance)
			{
				RenderSettings.fogStartDistance = PLTween.Tween(time, 0f, 1f, StartDistanceRemapZero, StartDistanceRemapOne, StartDistanceCurve);
			}

			if (ModifyEndDistance)
			{
				RenderSettings.fogEndDistance = PLTween.Tween(time, 0f, 1f, EndDistanceRemapZero, EndDistanceRemapOne, EndDistanceCurve);
			}
		}

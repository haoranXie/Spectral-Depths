...
namespace SpectralDepths.FeedbacksForThirdParty
{
	...
	[FeedbackHelp("This feedback will let you change the priorities of your cameras. It requires a bit of setup : " +
	              ...
	              "Optionally, you can add a PLCinemachinePriorityBrainListener on your Cinemachine Brain to handle different transition types and durations. " +
	              "Then all you have to do is pick a channel and a new priority on your feedback, and play it. Magic transition!")]
	public class PLFeedbackCinemachineTransition : PLFeedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (Mode == Modes.Event)
			{
				PLCinemachinePriorityEvent.Trigger(ChannelData(Channel), ForceMaxPriority, NewPriority, ForceTransition, _tempBlend, ResetValuesAfterTransition, Timing.TimescaleMode);    
			}
			else
			{
				PLCinemachinePriorityEvent.Trigger(ChannelData(Channel), ForceMaxPriority, 0, ForceTransition, _tempBlend, ResetValuesAfterTransition, Timing.TimescaleMode); 

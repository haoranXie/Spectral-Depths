...
namespace SpectralDepths.Feedbacks
{
	...
	#endif
	public class PLF_TMPCountTo : PLF_Feedback
	{
		...
		[PLFInspectorGroup("TextMeshPro Target Text", true, 12, true)]
		...
		[PLFInspectorGroup("Count Settings", true, 13)]
		...
		public PLTweenType CountingCurve = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		protected virtual float ProcessCount()
		{
			...
			float currentValue = PLTween.Tween(currentTime, 0f, Duration, CountFrom, CountTo, CountingCurve);
			return currentValue;
		}

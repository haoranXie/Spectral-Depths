...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLFeedbackTMPTextReveal : PLFeedback
	{
		...
		[PLFEnumCondition("DurationMode", (int)DurationModes.Interval)]
		...
		[PLFEnumCondition("DurationMode", (int)DurationModes.TotalDuration)]
		...
		protected virtual IEnumerator RevealLines()
		{
			...
			while (visibleLines <= totalLines)
			{
				...
				if (Timing.TimescaleMode == TimescaleModes.Scaled)
				{
					yield return PLFeedbacksCoroutine.WaitFor(_delay);
				}
				else
				{
					yield return PLFeedbacksCoroutine.WaitForUnscaled(_delay);
				}
			}
			IsPlaying = false;
			#else

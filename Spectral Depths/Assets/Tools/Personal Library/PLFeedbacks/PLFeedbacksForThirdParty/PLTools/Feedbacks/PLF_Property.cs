...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Property : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Target Property", true, 12)]
		/// the receiver to write the level to
		[Tooltip("the receiver to write the level to")]
		public PLPropertyReceiver Target;

		[PLFInspectorGroup("Mode", true, 29)]
		...
		[PLFEnumCondition("Mode", (int)Modes.OverTime, (int)Modes.ToDestination)]
		...
		[PLFInspectorGroup("Level", true, 30)]
		/// the curve to tween the intensity on
		[Tooltip("the curve to tween the intensity on")]
		[PLFEnumCondition("Mode", (int)Modes.OverTime, (int)Modes.ToDestination)]
		public PLTweenType LevelCurve = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		{
			float intensity = PLTween.Tween(time, 0f, 1f, remapZero, remapOne, LevelCurve);

...
{    
	/// <summary>
	/// This feedback will let you play a sound via the PLSoundManager. You will need a game object in your scene with a PLSoundManager object on it for this to work.
	/// </summary>
	[ExecuteAlways]
	[AddComponentMenu("")]
	[FeedbackPath("Audio/PLSoundManager Sound")]
	[FeedbackHelp("This feedback will let you play a sound via the PLSoundManager. You will need a game object in your scene with a PLSoundManager object on it for this to work.")]
	public class PLF_MMSoundManagerSound : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Sound", true, 14, true)]
		/// the sound clip to play
		[Tooltip("the sound clip to play")]
		public AudioClip Sfx;

		[PLFInspectorGroup("Random Sound", true, 34, true)]
		...
		[PLFCondition("SequentialOrder", true)]
		...
		[PLFCondition("SequentialOrderHoldLast", true)]
		...
		[PLFInspectorGroup("Scriptable Object", true, 14, true)]
		/// a scriptable object (created via the Create/SpectralDepths/Audio/PLF_SoundData menu) to define settings that will override all other settings on this feedback
		[Tooltip("a scriptable object (created via the Create/SpectralDepths/Audio/PLF_SoundData menu) to define settings that will override all other settings on this feedback")]
		public PLF_MMSoundManagerSoundData SoundDataSO;

		[PLFInspectorGroup("Sound Properties", true, 24)]
		[Header("Volume")]
		...
		[PLFInspectorGroup("SoundManager Options", true, 28)]
		...
		public PLSoundManager.PLSoundManagerTracks MmSoundManagerTrack = PLSoundManager.PLSoundManagerTracks.Sfx;
		...
		[PLFInspectorGroup("Fade", true, 30)]
		...
		[PLFInspectorGroup("Solo", true, 32)]
		...
		[PLFInspectorGroup("Spatial Settings", true, 33)]
		...
		[PLFInspectorGroup("Effects", true, 36)]
		...
		[PLFInspectorGroup("Time Options", true, 15)]
		...
		[PLFInspectorGroup("3D Sound Settings", true, 37)]
		...
		[PLFCondition("UseCustomRolloffCurve", true)]
		...
		[PLFCondition("UseSpatialBlendCurve", true)]
		...
		[PLFCondition("UseReverbZoneMixCurve", true)]
		...
		[PLFCondition("UseSpreadCurve", true)]
		public AnimationCurve SpreadCurve;
        
		[PLFInspectorGroup("Debug", true, 31)]
		/// whether or not to draw sound falloff gizmos when this PLF Player is selected
		[Tooltip("whether or not to draw sound falloff gizmos when this PLF Player is selected")]
		public bool DrawGizmos = false;
		/// an object to use as the center of the gizmos. If left empty, this PLF Player's position will be used.
		[Tooltip("an object to use as the center of the gizmos. If left empty, this PLF Player's position will be used.")]
		[PLFCondition("DrawGizmos", true)]
		...
		[PLFCondition("DrawGizmos", true)]
		public Color MinDistanceColor = PLColors.CadetBlue;
		...
		[PLFCondition("DrawGizmos", true)]
		public Color MaxDistanceColor = PLColors.Orangered;
		/// a test button used to play the sound in inspector
		public PLF_Button TestPlayButton;
		/// a test button used to stop the sound in inspector
		public PLF_Button TestStopButton;
		/// a test button used to stop the sound in inspector
		public PLF_Button ResetSequentialIndexButton;
        
		protected AudioClip _randomClip;
		protected AudioSource _editorAudioSource;
		protected PLSoundManagerPlayOptions _options;
		...
		protected PLShufflebag<int> _randomUniqueShuffleBag;
		...
		protected override void CustomInitialization(PLF_Player owner)
		{
			...
			if (RandomUnique)
			{
				_randomUniqueShuffleBag = new PLShufflebag<int>(RandomSfx.Length);
		...
		{
			TestPlayButton = new PLF_Button("Debug Play Sound", TestPlaySound);
			TestStopButton = new PLF_Button("Debug Stop Sound", TestStopSound);
			ResetSequentialIndexButton = new PLF_Button("Reset Sequential Index", ResetSequentialIndex);
		}
		...
		protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1)
		{
			...
			if (StopSoundOnFeedbackStop && (_playedAudioSource != null))
			{
				...
				PLSoundManager.Instance.FreeSound(_playedAudioSource);
			}
		}
		...
		protected virtual void PlaySound(AudioClip sfx, Vector3 position, float intensity)
		{
			...
			{
				if ((PLSoundManager.Instance.FindByClip(sfx) != null) && (PLSoundManager.Instance.FindByClip(sfx).isPlaying))
				{
					return;
				}
			}
			...
			_playedAudioSource = PLSoundManagerSoundPlayEvent.Trigger(sfx, _options);

...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Sound : PLF_Feedback
	{
		...
		/// Event : sends a PLSfxEvent, you'll need a class to catch this event and play the sound
		...
		[PLFInspectorGroup("Play Method", true, 27)]
		...
		[PLFEnumCondition("PlayMethod", (int)PlayMethods.Pool)]
		...
		[PLFInspectorGroup("Sound Properties", true, 28)]
		...
		[PLFInspectorGroup("Spatial Settings", true, 33, false, true)]
		...
		[PLFInspectorGroup("3D Sound Settings", true, 37, false, true)]
		...
		protected virtual void PlaySound(AudioClip sfx, Vector3 position, float intensity)
		{
			...
			switch (PlayMethod)
			{
				case PlayMethods.Event:
					PLSfxEvent.Trigger(sfx, SfxAudioMixerGroup, volume, pitch, Priority);
					break;

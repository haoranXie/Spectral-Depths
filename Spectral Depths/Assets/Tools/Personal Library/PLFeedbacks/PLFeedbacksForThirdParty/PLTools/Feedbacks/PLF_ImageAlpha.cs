...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_ImageAlpha : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Target Image", true, 12, true)]
		...
		[PLFInspectorGroup("Image Alpha Animation", true, 24)]
		...
		protected virtual void SetAlpha(float time)
		{
			...
			{
				newAlpha = PLTween.Tween(time, 0f, 1f, CurveRemapZero, CurveRemapOne, Curve);    
			}
			else if (Mode == Modes.ToDestination)
			{
				newAlpha = PLTween.Tween(time, 0f, 1f, _initialAlpha, DestinationAlpha, Curve);
			}

...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_ImageFill : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Image Fill Animation", true, 24)]
		...
		protected virtual void SetFill(float time)
		{
			...
			{
				newFill = PLTween.Tween(time, 0f, 1f, CurveRemapZero, CurveRemapOne, Curve);    
			}
			else if (Mode == Modes.ToDestination)
			{
				newFill = PLTween.Tween(time, 0f, 1f, _initialFill, DestinationFill, Curve);
			}

...
{
	public class PLF_FeedbackBaseTarget
	{
	...
	public abstract class PLF_FeedbackBase : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Mode", true, 64)]
		...
		protected List<PLF_FeedbackBaseTarget> _targets;
		...
		{
			_targets = new List<PLF_FeedbackBaseTarget>();
			FillTargets();
			InitializeTargets();
		}
		...
		protected virtual void InitializeTargets()
		{
			...
			foreach(PLF_FeedbackBaseTarget target in _targets)
			{
		...
		protected virtual void SetValues(float time, float feedbacksIntensity, Vector3 position)
		{
			...
			foreach (PLF_FeedbackBaseTarget target in _targets)
			{
				float intensity = PLTween.Tween(time, 0f, 1f, target.RemapLevelZero, target.RemapLevelOne, target.LevelCurve);
				if (RelativeValues)
				{

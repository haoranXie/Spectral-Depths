...
namespace SpectralDepths.FeedbacksForThirdParty
{
	...
	#endif
	[AddComponentMenu("Spectral Depths/Feedbacks/Shakers/PostProcessing/PLColorAdjustmentsShaker_HDRP")]
	public class PLColorAdjustmentsShaker_HDRP : PLShaker
	{
		/// whether or not to add to the initial value
		public bool RelativeValues = true;

		[PLInspectorGroup("Post Exposure", true, 44)]
		...
		[PLInspectorGroup("Hue Shift", true, 45)]
		...
		[PLInspectorGroup("Saturation", true, 46)]
		...
		[PLInspectorGroup("Contrast", true, 47)]
		/// the curve used to animate the focal length value on
		[Tooltip("the curve used to animate the focal length value on")]
		...
		[PLInspectorGroup("Color Filter", true, 48)]
		...
		[PLFEnumCondition("ColorFilterMode", (int)ColorFilterModes.Gradient)]
		[GradientUsage(true)]
		...
		[PLFEnumCondition("ColorFilterMode", (int) ColorFilterModes.Interpolate)]
		...
		protected override void Shake()
		{
			...
			_remappedTimeSinceStart = PLFeedbacksHelpers.Remap(Time.time - _shakeStartedTimestamp, 0f, ShakeDuration, 0f, 1f);
		...
		public virtual void OnMMColorGradingShakeEvent(AnimationCurve shakePostExposure, float remapPostExposureZero, float remapPostExposureOne,
			...
			float attenuation = 1.0f, PLChannelData channelData = null, bool resetShakerValuesAfterShake = true, bool resetTargetValuesAfterShake = true, 
		...
		{
			base.StartListening();
			PLColorAdjustmentsShakeEvent_HDRP.Register(OnMMColorGradingShakeEvent);
		}
		...
		{
			base.StopListening();
			PLColorAdjustmentsShakeEvent_HDRP.Unregister(OnMMColorGradingShakeEvent);
		}
		#endif
	}
	...
	/// </summary>
	public struct PLColorAdjustmentsShakeEvent_HDRP
	{
		...
		public delegate void Delegate(AnimationCurve shakePostExposure, float remapPostExposureZero, float remapPostExposureOne,
			...
			AnimationCurve shakeSaturation, float remapSaturationZero, float remapSaturationOne,
			AnimationCurve shakeContrast, float remapContrastZero, float remapContrastOne,
			PLColorAdjustmentsShaker_HDRP.ColorFilterModes colorFilterMode, Gradient colorFilterGradient, Color colorFilterDestination,AnimationCurve colorFilterCurve,  
			float duration, bool relativeValues = false,
			float attenuation = 1.0f, PLChannelData channelData = null, bool resetShakerValuesAfterShake = true, bool resetTargetValuesAfterShake = true, 
			bool forwardDirection = true, TimescaleModes timescaleMode = TimescaleModes.Scaled, bool stop = false, bool restore = false);

...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	public enum PLTimeScaleMethods
	{
		For,
		Reset,
		Unfreeze
	...
	/// </summary>
	public enum PLTimeScaleLerpModes { Speed, Duration, NoInterpolation } 
	...
	public struct TimeScaleProperties
	{
		...
		public PLTimeScaleLerpModes TimeScaleLerpMode;
		public PLTweenType TimeScaleLerpCurve;
		public float TimeScaleLerpDuration;
		public bool TimeScaleLerpOnReset;
		public PLTweenType TimeScaleLerpCurveOnReset;
	...
	public struct PLTimeScaleEvent
	{
		...
		public delegate void Delegate(PLTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp, float lerpSpeed, bool infinite,
			PLTimeScaleLerpModes timeScaleLerpMode = PLTimeScaleLerpModes.Speed, PLTweenType timeScaleLerpCurve = null, float timeScaleLerpDuration = 0.2f, 
			bool timeScaleLerpOnReset = false, PLTweenType timeScaleLerpCurveOnReset = null, float timeScaleLerpDurationOnReset = 0.2f);

		static public void Trigger(PLTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp, float lerpSpeed, bool infinite,
			PLTimeScaleLerpModes timeScaleLerpMode = PLTimeScaleLerpModes.Speed, PLTweenType timeScaleLerpCurve = null, float timeScaleLerpDuration = 0.2f, 
			bool timeScaleLerpOnReset = false, PLTweenType timeScaleLerpCurveOnReset = null, float timeScaleLerpDurationOnReset = 0.2f)
		{
	...
	public struct PLFreezeFrameEvent
	{
		static private event Delegate OnEvent;
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)] private static void RuntimeInitialization() { OnEvent = null; }
		static public void Register(Delegate callback) { OnEvent += callback; }
	...
	/// <summary>
	/// Put this component in your scene and it'll catch PLFreezeFrameEvents and PLTimeScaleEvents, allowing you to control the flow of time.
	/// </summary>
	[AddComponentMenu("Spectral Depths/Feedbacks/Shakers/Various/PLTimeManager")]
	public class PLTimeManager : PLSingleton<PLTimeManager>
	{	
		[Header("Default Values")]
		[PLFInformationAttribute("Put this component in your scene and it'll catch PLFreezeFrameEvents and PLTimeScaleEvents, allowing you to control the flow of time.", PLFInformationAttribute.InformationType.Info, false)]
		...
		[PLFInspectorButtonAttribute("TestButtonToSlowDownTime")]
		...
		{
			PLTimeScaleEvent.Trigger(PLTimeScaleMethods.For, 0.5f, 3f, true, 1f, false);
		}
		...
		protected virtual void Update()
		{
			...
			{
				if (_currentProperty.TimeScaleLerpMode == PLTimeScaleLerpModes.Speed)
				{
				...
				else if (_currentProperty.TimeScaleLerpMode == PLTimeScaleLerpModes.Duration)
				{
					float timeSinceStart = Time.unscaledTime - _startedAt;
					float progress = PLMaths.Remap(timeSinceStart, 0f, _currentProperty.TimeScaleLerpDuration, 0f, 1f);
		...
		protected virtual void SetTimeScale(TimeScaleProperties timeScaleProperties)
		{
			if (timeScaleProperties.TimeScaleLerp &&
			    timeScaleProperties.TimeScaleLerpMode == PLTimeScaleLerpModes.Duration)
			{
		...
		protected virtual void Unfreeze()
		{
			...
			if (_timeScaleProperties.Count == 0)
			{
				if (_resetProperty.TimeScaleLerp && _resetProperty.TimeScaleLerpMode == PLTimeScaleLerpModes.Duration && _resetProperty.TimeScaleLerpOnReset)
				{
					_lerpingBackToNormal = true;
					PLTimeScaleEvent.Trigger(PLTimeScaleMethods.For, NormalTimeScale, _resetProperty.TimeScaleLerpDuration, _resetProperty.TimeScaleLerp, 
						_resetProperty.LerpSpeed, true, PLTimeScaleLerpModes.Duration, _resetProperty.TimeScaleLerpCurveOnReset, _resetProperty.TimeScaleLerpDurationOnReset);	
				}
				else
				{
					ResetTimeScale();	
		...
		{
			PLTimeScaleEvent.Trigger(PLTimeScaleMethods.For, newNormalTimeScale, 0f, false, 0f, true);
		}

		/// <summary>
		/// Catches TimeScaleEvents and acts on them
		/// </summary>
		/// <param name="timeScaleEvent">PLTimeScaleEvent event.</param>
		public virtual void OnTimeScaleEvent(PLTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp, float lerpSpeed, bool infinite,
			PLTimeScaleLerpModes timeScaleLerpMode = PLTimeScaleLerpModes.Speed, PLTweenType timeScaleLerpCurve = null, float timeScaleLerpDuration = 0.2f, 
			bool timeScaleLerpOnReset = false, PLTweenType timeScaleLerpCurveOnReset = null, float timeScaleLerpDurationOnReset = 0.2f)
		{
			...
			{
				case PLTimeScaleMethods.Reset:
					ResetTimeScale ();
					break;

				case PLTimeScaleMethods.For:
					SetTimeScale (timeScaleProperty);
					break;

				case PLTimeScaleMethods.Unfreeze:
					Unfreeze();
					break;
			}
		}
		...
		void OnEnable()
		{
			PLFreezeFrameEvent.Register(OnMMFreezeFrameEvent);
			PLTimeScaleEvent.Register(OnTimeScaleEvent);
		}
		...
		void OnDisable()
		{
			PLFreezeFrameEvent.Unregister(OnMMFreezeFrameEvent);
			PLTimeScaleEvent.Unregister(OnTimeScaleEvent);
		}		
	}
}
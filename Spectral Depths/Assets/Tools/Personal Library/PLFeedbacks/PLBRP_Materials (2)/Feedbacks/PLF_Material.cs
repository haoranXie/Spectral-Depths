...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Material : PLF_Feedback
	{
		/// sets the inspector color for this feedback
		#if UNITY_EDITOR
		public override Color FeedbackColor { get => PLFeedbacksInspectorColors.RendererColor; }
		...
		[PLFInspectorGroup("Target Material", true, 61, true)]
		...
		[PLFInspectorGroup("Interpolation", true, 35)]
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			{
				Debug.LogError("[PLFeedbackMaterial on " + Owner.name + "] The Materials array is empty.");
				return;
			}
			...
			{
				Debug.LogError("[PLFeedbackMaterial on " + Owner.name + "] Attempting to switch to a null material.");
				return;
			}
		...
		protected virtual IEnumerator TransitionMaterial(Material originalMaterial, Material newMaterial, int materialIndex)
		{
			...
			{
				float time = PLFeedbacksHelpers.Remap(GetTime() - _startedAt, 0f, TransitionDuration, 0f, 1f);

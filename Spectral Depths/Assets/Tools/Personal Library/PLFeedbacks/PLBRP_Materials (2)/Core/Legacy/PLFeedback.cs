...
{
	/// <summary>
	/// A base class, meant to be extended, defining a Feedback. A Feedback is an action triggered by a PLFeedbacks, usually in reaction to the player's input or actions,
	...
	[System.Serializable]
	[ExecuteAlways]
	public abstract class PLFeedback : MonoBehaviour
	{
		...
		public virtual PLChannelData ChannelData(int channel) => _channelData.Set(PLChannelModes.Int, channel, null);
		...
		protected PLFeedbacks _hostMMFeedbacks;
		...
		{
			_hostMMFeedbacks = this.gameObject.GetComponent<PLFeedbacks>();
		...
		public virtual void Initialization(GameObject owner)
		{
			...
			_hostMMFeedbacks = this.gameObject.GetComponent<PLFeedbacks>();
			_channelData = new PLChannelData(PLChannelModes.Int, 0, null);
		...
		protected virtual IEnumerator PlayCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			if (Timing.TimescaleMode == TimescaleModes.Scaled)
			{
				yield return PLFeedbacksCoroutine.WaitFor(Timing.InitialDelay);
			}
			else
			{
				yield return PLFeedbacksCoroutine.WaitForUnscaled(Timing.InitialDelay);
			}
			_lastPlayTimestamp = FeedbackTime;
			RegularPlay(position, feedbacksIntensity);
		}
		...
		protected virtual IEnumerator InfinitePlay(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			while (true)
			{
				...
				if (Timing.Sequence == null)
				{
					...
					if (Timing.TimescaleMode == TimescaleModes.Scaled)
					{
						yield return PLFeedbacksCoroutine.WaitFor(Timing.DelayBetweenRepeats);
					}
					else
					{
						yield return PLFeedbacksCoroutine.WaitForUnscaled(Timing.DelayBetweenRepeats);
					}
				}
				else
				{
			...
			get
			{
				switch (Timing.PlayDirection)
				{
					case PLFeedbackTiming.PlayDirections.FollowMMFeedbacksDirection:
						return (_hostMMFeedbacks.Direction == PLFeedbacks.Directions.TopToBottom);
			...
			get
			{
				switch (Timing.PLFeedbacksDirectionCondition)
				{
					...
					case PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenBackwards:
						return (_hostMMFeedbacks.Direction == PLFeedbacks.Directions.BottomToTop);
				}
				return true;
			}
		}

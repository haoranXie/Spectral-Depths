...
namespace SpectralDepths.Feedbacks
{
	...
	/// This is designed to be spawned by a PLFloatingTextSpawner, not used on its own.
	/// It also requires a specific hierarchy. You'll find examples of it in the PLTools/Tools/PLFloatingText/Prefabs folder
	/// </summary>
	public class PLFloatingText : MonoBehaviour
	{
		[Header("Bindings")]
		...
		protected PLFloatingTextSpawner.AlignmentModes _alignmentMode;
		...
		protected virtual void UpdateFloatingText()
		{
			...
			_remappedTime = PLMaths.Remap(_elapsedTime, 0f, _lifetime, 0f, 1f);
		...
		protected virtual void HandleMovement()
		{
			...
			if (_animateMovement)
			{
				...
				_newPosition.x = _animateX ? PLMaths.Remap(_animateXCurve.Evaluate(_remappedTime), 0f, 1, _remapXZero, _remapXOne) : 0f;
				_newPosition.y = _animateY ? PLMaths.Remap(_animateYCurve.Evaluate(_remappedTime), 0f, 1, _remapYZero, _remapYOne) : 0f;
				_newPosition.z = _animateZ ? PLMaths.Remap(_animateZCurve.Evaluate(_remappedTime), 0f, 1, _remapZZero, _remapZOne) : 0f;
		...
		protected virtual void HandleOpacity()
		{
			if (_animateOpacity)
			{
				float newOpacity = PLMaths.Remap(_animateOpacityCurve.Evaluate(_remappedTime), 0f, 1f, _remapOpacityZero, _remapOpacityOne);
		...
		protected virtual void HandleScale()
		{
			if (_animateScale)
			{
				_newScale = Vector3.one * PLMaths.Remap(_animateScaleCurve.Evaluate(_remappedTime), 0f, 1f, _remapScaleZero, _remapScaleOne);
		...
		{
			if (_alignmentMode == PLFloatingTextSpawner.AlignmentModes.Fixed)
			{
				MovingPart.transform.up = _fixedAlignment;
			}
			else if (_alignmentMode == PLFloatingTextSpawner.AlignmentModes.MatchInitialDirection)
			{
				MovingPart.transform.up = this.transform.up;
			}
			else if (_alignmentMode == PLFloatingTextSpawner.AlignmentModes.MatchMovementDirection)
			{
		...
		public virtual void SetProperties(string value, float lifetime, Vector3 direction, bool animateMovement, 
			PLFloatingTextSpawner.AlignmentModes alignmentMode, Vector3 fixedAlignment,

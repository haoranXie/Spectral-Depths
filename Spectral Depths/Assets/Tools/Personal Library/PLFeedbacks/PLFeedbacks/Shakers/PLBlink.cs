...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Feedbacks/Shakers/Various/PLBlink")]
	public class PLBlink : MonoBehaviour
	{
		...
		[PLFEnumCondition("Method", (int)Methods.SetGameObjectActive)]
		public GameObject TargetGameObject;
		/// the target renderer to work with
		[Tooltip("the target renderer to work with")]
		[PLFEnumCondition("Method", (int)Methods.MaterialAlpha, (int)Methods.MaterialEmissionIntensity, (int)Methods.ShaderFloatValue)]
		...
		[PLFCondition("ForceStateOnExit", true)]
		public States StateOnExit = States.On;

		[Header("Timescale")] 
		/// whether or not this PLBlink should operate on unscaled time 
		[Tooltip("whether or not this PLBlink should operate on unscaled time")]
		...
		/// Test button
		[Tooltip("Test button")]
		[PLFInspectorButton("ToggleBlinking")]
		public bool ToggleBlinkingButton;
		/// Test button
		[Tooltip("Test button")]
		[PLFInspectorButton("StartBlinking")]
		public bool StartBlinkingButton;
		/// Test button
		[Tooltip("Test button")]
		[PLFInspectorButton("StopBlinking")]
		...
		protected virtual void Blink()
		{
			...
			{
				float t = PLFeedbacksHelpers.Remap(GetTime() - _lastBlinkAt, 0f, _currentLerpDuration, 0f, 1f);
				newValue = Curve.Evaluate(t);
				newValue = PLFeedbacksHelpers.Remap(newValue, 0f, 1f, initialValue, targetValue);
			}
			else
			{
		...
		protected virtual void InitializeBlinkProperties()
		{
			if (Phases.Count == 0)
			{
				Debug.LogError("PLBlink : You need to define at least one phase for this component to work.");
				this.enabled = false;
				return;
			}

...
{
	/// <summary>
	/// Add this to an object and it'll be able to listen for PLFLookAtShakeEvents, and when one is received, it will rotate its associated transform accordingly
	/// </summary>
	public class PLLookAtShaker : PLShaker
	{
		[PLInspectorGroup("Look at settings", true, 37)]
		...
		public PLTweenType LookAtTween = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)));
		...
		[PLInspectorGroup("What we want to rotate", true, 37)]
		...
		public PLF_LookAt.UpwardVectors UpwardVector = PLF_LookAt.UpwardVectors.Up;

		[PLInspectorGroup("What we want to look at", true, 37)]
		...
		public PLF_LookAt.LookAtTargetModes LookAtTargetMode = PLF_LookAt.LookAtTargetModes.Transform;
		/// the transform we want to look at 
		[Tooltip("the transform we want to look at")]
		[PLFEnumCondition("LookAtTargetMode", (int)PLF_LookAt.LookAtTargetModes.Transform)]
		...
		[PLFEnumCondition("LookAtTargetMode", (int)PLF_LookAt.LookAtTargetModes.TargetWorldPosition)]
		...
		[PLFEnumCondition("LookAtTargetMode", (int)PLF_LookAt.LookAtTargetModes.Direction)]
		public Vector3 LookAtDirection = Vector3.forward;
		
		[PLInspectorGroup("Test", true, 46)]
		[PLInspectorButton("StartShaking")] 
		public bool StartShakingButton;
		
		/// <summary>
		/// An event used to trigger a look at shake 
		/// </summary>
		public struct PLLookAtShakeEvent
		{
			...
			public delegate void Delegate(float duration, 
				bool lockXAxis, bool lockYAxis, bool lockZAxis, PLF_LookAt.UpwardVectors upwardVector, PLF_LookAt.LookAtTargetModes lookAtTargetMode,Transform lookAtTarget, Vector3 lookAtTargetWorldPosition, Vector3 lookAtDirection, Transform transformToRotate, PLTweenType lookAtTween,
				...
				float feedbacksIntensity = 1.0f, PLChannelData channelData = null, bool resetShakerValuesAfterShake = true, 
		...
		protected PLTweenType _originalLookAtTween;
		protected bool _originalLockXAxis;
		protected bool _originalLockYAxis;
		protected bool _originalLockZAxis;
		protected PLF_LookAt.UpwardVectors _originalUpwardVector;
		protected PLF_LookAt.LookAtTargetModes _originalLookAtTargetMode;
		...
		protected virtual void ApplyRotation(float journey)
		{
			...
			{
				case PLF_LookAt.LookAtTargetModes.Transform:
					_lookAtPosition = LookAtTarget.position;
					break;
				case PLF_LookAt.LookAtTargetModes.TargetWorldPosition:
					_lookAtPosition = LookAtTargetWorldPosition;
					break;
				case PLF_LookAt.LookAtTargetModes.Direction:
		...
		{
			base.StartListening();
			PLLookAtShakeEvent.Register(OnMMLookAtShakeEvent);
		}
		...
		{
			base.StopListening();
			PLLookAtShakeEvent.Unregister(OnMMLookAtShakeEvent);
		}
	}
}
...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Feedbacks/Shakers/Various/PLWiggle")]
	public class PLWiggle : MonoBehaviour 
	{
		...
		{
			returnValue.x = PLFeedbacksHelpers.Remap(curve.Evaluate(percent), 0f, 1f, remapMin.x, remapMax.x);
			returnValue.y = PLFeedbacksHelpers.Remap(curve.Evaluate(percent), 0f, 1f, remapMin.y, remapMax.y);
			returnValue.z = PLFeedbacksHelpers.Remap(curve.Evaluate(percent), 0f, 1f, remapMin.z, remapMax.z);

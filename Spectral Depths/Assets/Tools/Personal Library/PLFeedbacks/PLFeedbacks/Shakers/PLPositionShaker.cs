...
namespace SpectralDepths.Feedbacks
{
	...
	/// </summary>
	public class PLPositionShaker : PLShaker
	{
		public enum Modes { Transform, RectTransform }

		[PLInspectorGroup("Target", true, 41)]
		...
		[PLEnumCondition("Mode", (int)Modes.Transform)]
		...
		[PLEnumCondition("Mode", (int)Modes.RectTransform)]
		public RectTransform TargetRectTransform;

		[PLInspectorGroup("Shake Settings", true, 42)]
		...
		[PLInspectorGroup("Direction", true, 43)]
		...
		[PLCondition("RandomizeDirection", true)]
		...
		[PLInspectorGroup("Directional Noise", true, 47)]
		...
		[PLCondition("AddDirectionalNoise", true)]
		...
		[PLInspectorGroup("Randomness", true, 44)]
		...
		[PLInspectorGroup("One Time", true, 45)]
		...
		[PLCondition("UseAttenuation", true)]
		...
		protected override void ShakeStarts()
		{
			...
			_randomizedDirection = RandomizeDirection ? PLMaths.RandomVector3(ShakeMainDirection, ShakeAltDirection) : ShakeMainDirection;
		}
        
		protected override void Shake()
		{
			_oscillation = OscillationOffset + Mathf.Sin(ShakeSpeed * (Randomness + _journey));
			float remappedTime = PLFeedbacksHelpers.Remap(_journey, 0f, ShakeDuration, 0f, 1f);
		...
		protected virtual Vector3 ComputeNoise(float time)
		{
			...
			_randomNoiseStrength = PLMaths.RandomVector3(DirectionalNoiseStrengthMin, DirectionalNoiseStrengthMax); 
		...
		{
			base.StartListening();
			PLPositionShakeEvent.Register(OnMMPositionShakeEvent);
		}
		...
		{
			base.StopListening();
			PLPositionShakeEvent.Unregister(OnMMPositionShakeEvent);
		}
	}
	
	public struct PLPositionShakeEvent
	{

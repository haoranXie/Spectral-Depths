...
{
	/// <summary>
	/// This feedback will let you change the color of a target sprite renderer over time, and flip it on X or Y. You can also use it to command one or many PLSpriteRendererShakers.
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("This feedback will let you change the color of a target sprite renderer over time, and flip it on X or Y. You can also use it to command one or many PLSpriteRendererShakers.")]
	[FeedbackPath("Renderer/SpriteRenderer")]
	public class PLF_SpriteRenderer : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Sprite Renderer", true, 51, true)]
		...
		[PLFEnumCondition("Mode", (int)Modes.OverTime, (int)Modes.ShakerEvent, (int)Modes.ToDestinationColor, (int)Modes.ToDestinationColorAndBack)]
		...
		[PLFInspectorGroup("Color", true, 52)]
		...
		[Tooltip("the color to move to in instant mode")]
		[PLFEnumCondition("Mode", (int)Modes.Instant, (int)Modes.ToDestinationColor, (int)Modes.ToDestinationColorAndBack)]
		...
		[PLFInspectorGroup("Flip", true, 53)]
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			switch (Mode)
			{
				...
				case Modes.ShakerEvent:
					PLSpriteRendererShakeEvent.Trigger(FeedbackDuration, ModifyColor, ColorOverTime, 
						FlipX, FlipY,   
						intensityMultiplier,
			...
			while ((journey >= 0) && (journey <= FeedbackDuration) && (FeedbackDuration > 0))
			{
				...
				if (andBack)
				{
					remappedTime = (remappedTime < 0.5f)
						? PLFeedbacksHelpers.Remap(remappedTime, 0f, 0.5f, 0f, 1f)
						: PLFeedbacksHelpers.Remap(remappedTime, 0.5f, 1f, 1f, 0f);
				}

...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLFeedbackFlicker : PLFeedback
	{
		...
		[PLFEnumCondition("Mode", (int)Modes.PropertyName)]
		...
		protected override void CustomInitialization(GameObject owner)
		{
			...
			{
				if (owner.PLFGetComponentNoAlloc<Renderer>() != null)
				{
			...
			if (BoundRenderer == null)
			{
				Debug.LogWarning("[PLFeedbackFlicker] The flicker feedback on "+this.name+" doesn't have a bound renderer, it won't work. You need to specify a renderer to flicker in its inspector.");    
			}
		...
		public virtual IEnumerator Flicker(Renderer renderer, int materialIndex, Color initialColor, Color flickerColor, float flickerSpeed, float flickerDuration)
		{
			...
			while (FeedbackTime < flickerStop)
			{
				...
				{
					yield return PLFeedbacksCoroutine.WaitFor(flickerSpeed);
				}
				else
				{
					yield return PLFeedbacksCoroutine.WaitForUnscaled(flickerSpeed);
				}

...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Scale : PLF_Feedback
	{
		...
		#if UNITY_EDITOR
		public override Color FeedbackColor { get { return PLFeedbacksInspectorColors.TransformColor; } }
		...
		[PLFInspectorGroup("Scale Mode", true, 12, true)]
		...
		[PLFInspectorGroup("Scale Animation", true, 13)]
		...
		public PLTweenType AnimateScaleTweenX = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)));
		...
		public PLTweenType AnimateScaleTweenY = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)));
		...
		public PLTweenType AnimateScaleTweenZ = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)));
		...
		[PLFEnumCondition("Mode", (int)Modes.ToDestination)]
		...
		protected virtual IEnumerator ScaleToDestination()
		{
			...
			while ((journey >= 0) && (journey <= FeedbackDuration) && (FeedbackDuration > 0))
			{
				...
				if (AnimateX)
				{
					...
					_newScale.x = PLFeedbacksHelpers.Remap(_newScale.x, 0f, 1f, RemapCurveZero, RemapCurveOne);
				}

				if (AnimateY)
				{
					_newScale.y = Mathf.LerpUnclamped(_initialScale.y, DestinationScale.y, AnimateScaleTweenY.Evaluate(percent) + Offset);
					_newScale.y = PLFeedbacksHelpers.Remap(_newScale.y, 0f, 1f, RemapCurveZero, RemapCurveOne);    
				}

				if (AnimateZ)
				{
					_newScale.z = Mathf.LerpUnclamped(_initialScale.z, DestinationScale.z, AnimateScaleTweenZ.Evaluate(percent) + Offset);
					_newScale.z = PLFeedbacksHelpers.Remap(_newScale.z, 0f, 1f, RemapCurveZero, RemapCurveOne);    
				}
		...
		/// <returns></returns>
		protected virtual IEnumerator AnimateScale(Transform targetTransform, Vector3 vector, float duration, PLTweenType curveX, PLTweenType curveY, PLTweenType curveZ, float remapCurveZero = 0f, float remapCurveOne = 1f)
		{
			...
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				...
				if (AnimateX)
				{
					...
					vector.x = PLFeedbacksHelpers.Remap(vector.x, 0f, 1f, remapCurveZero, remapCurveOne);
				...
				if (AnimateY)
				{
					...
					vector.y = PLFeedbacksHelpers.Remap(vector.y, 0f, 1f, remapCurveZero, remapCurveOne);    
					if (Mode == Modes.Additive)
					{
				...
				if (AnimateZ)
				{
					...
					vector.z = PLFeedbacksHelpers.Remap(vector.z, 0f, 1f, remapCurveZero, remapCurveOne);    
					if (Mode == Modes.Additive)
					{
		...
		{
			base.OnValidate();
			PLFeedbacksHelpers.MigrateCurve(AnimateScaleX, AnimateScaleTweenX, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimateScaleY, AnimateScaleTweenY, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimateScaleZ, AnimateScaleTweenZ, Owner);
		}

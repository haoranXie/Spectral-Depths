...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Rotation : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Rotation Target", true, 61, true)]
		...
		[PLFEnumCondition("Mode", (int)Modes.Absolute, (int)Modes.Additive)]
		...
		[PLFCondition("AnimateX")]
		public PLTweenType AnimateRotationTweenX = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[PLFCondition("AnimateY")]
		public PLTweenType AnimateRotationTweenY = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[PLFCondition("AnimateZ")]
		public PLTweenType AnimateRotationTweenZ = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)));
		...
		[Tooltip("how the x part of the rotation should animate over time, in degrees")]
		[PLFEnumCondition("Mode", (int)Modes.ToDestination)]
		public PLTweenType ToDestinationTween = new PLTweenType(PLTween.PLTweenCurve.EaseInQuintic);
		...
		protected virtual void ApplyRotation(Transform targetTransform, float remapZero, float remapOne, PLTweenType curveX, PLTweenType curveY, PLTweenType curveZ, float percent)
		{
			...
			if (AnimateX)
			{
				float x = PLTween.Tween(percent, 0f, 1f, remapZero, remapOne, curveX);
				targetTransform.Rotate(Vector3.right, x, RotationSpace);
			}
			if (AnimateY)
			{
				float y = PLTween.Tween(percent, 0f, 1f, remapZero, remapOne, curveY);
				targetTransform.Rotate(Vector3.up, y, RotationSpace);
			}
			if (AnimateZ)
			{
				float z = PLTween.Tween(percent, 0f, 1f, remapZero, remapOne, curveZ);
		...
		{
			base.OnValidate();
			PLFeedbacksHelpers.MigrateCurve(AnimateRotationX, AnimateRotationTweenX, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimateRotationY, AnimateRotationTweenY, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimateRotationZ, AnimateRotationTweenZ, Owner);
			PLFeedbacksHelpers.MigrateCurve(ToDestinationCurve, ToDestinationTween, Owner);
		}

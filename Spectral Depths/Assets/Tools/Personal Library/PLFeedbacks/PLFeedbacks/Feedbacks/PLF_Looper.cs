...
{
	/// <summary>
	/// This feedback will move the current "head" of an PLFeedbacks sequence back to another feedback above in the list.
	...
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp("This feedback will move the current 'head' of an PLFeedbacks sequence back to another feedback above in the list. " +
	...
	public class PLF_Looper : PLF_Pause
	{
		[PLFInspectorGroup("Loop", true, 34)]
        
		[Header("Loop conditions")]
		/// if this is true, this feedback, when met, will cause the PLFeedbacks to reposition its 'head' to the first pause found above it (going from this feedback to the top), or to the start if none is found
		[Tooltip("if this is true, this feedback, when met, will cause the PLFeedbacks to reposition its 'head' to the first pause found above it (going from this feedback to the top), or to the start if none is found")]
		public bool LoopAtLastPause = true;
		/// if this is true, this feedback, when met, will cause the PLFeedbacks to reposition its 'head' to the first LoopStart feedback found above it (going from this feedback to the top), or to the start if none is found
		[Tooltip("if this is true, this feedback, when met, will cause the PLFeedbacks to reposition its 'head' to the first LoopStart feedback found above it (going from this feedback to the top), or to the start if none is found")]
		...
		#if UNITY_EDITOR
		public override Color FeedbackColor { get { return PLFeedbacksInspectorColors.LooperColor; } }
		#endif

...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Position : PLF_Feedback
	{
		/// a static bool used to disable all feedbacks of this type at once
		public static bool FeedbackTypeAuthorized = true;
		/// sets the inspector color for this feedback
		#if UNITY_EDITOR
		public override Color FeedbackColor => PLFeedbacksInspectorColors.TransformColor; 
		...
		[PLFInspectorGroup("Position Target", true, 61, true)]
		...
		[PLFInspectorGroup("Transition", true, 63)]
		...
		[PLFEnumCondition("Mode", (int)Modes.AlongCurve)]
		...
		/// the PLTween curve definition to use instead of the animation curve to define the acceleration of the movement
		[Tooltip("the PLTween curve definition to use instead of the animation curve to define the acceleration of the movement")]
		[PLFEnumCondition("Mode", (int)Modes.AtoB, (int)Modes.ToDestination)]
		public PLTweenType AnimatePositionTween = new PLTweenType( new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)));
		...
		[PLFCondition("RandomizeRemap", true)]
		[PLFEnumCondition("Mode", (int)Modes.AlongCurve)]
		...
		[PLFCondition("AnimateX", true)]
		public PLTweenType AnimatePositionTweenX = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f)));
		...
		/// the acceleration of the movement
		[Tooltip("the acceleration of the movement")]
		[PLFCondition("AnimateY", true)]
		public PLTweenType AnimatePositionTweenY = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f)));
		...
		/// the acceleration of the movement
		[Tooltip("the acceleration of the movement")]
		[PLFCondition("AnimateZ", true)]
		public PLTweenType AnimatePositionTweenZ = new PLTweenType(new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(0.6f, -1f), new Keyframe(1, 0f)));
		/// if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over
		[Tooltip("if this is true, calling that feedback will trigger it, even if it's in progress. If it's false, it'll prevent any new Play until the current one is over")] 
		public bool AllowAdditivePlays = false;
		[PLFInspectorGroup("Positions", true, 64)]
		...
		[PLFEnumCondition("Mode", (int)Modes.AtoB, (int)Modes.AlongCurve)]
		...
		{
			float newValueX = PLTween.Tween(percent, 0f, 1f, _remapCurveZero * intensityMultiplier, _remapCurveOne * intensityMultiplier, AnimatePositionTweenX);
			float newValueY = PLTween.Tween(percent, 0f, 1f, _remapCurveZero * intensityMultiplier, _remapCurveOne * intensityMultiplier, AnimatePositionTweenY);
			float newValueZ = PLTween.Tween(percent, 0f, 1f, _remapCurveZero * intensityMultiplier, _remapCurveOne * intensityMultiplier, AnimatePositionTweenZ);
		...
		protected virtual IEnumerator MoveFromTo(GameObject movingObject, Vector3 pointA, Vector3 pointB, float duration, PLTweenType tweenType)
		{
			IsPlaying = true;
			float journey = NormalPlayDirection ? 0f : duration;
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				float curveValue = PLTween.Tween(journey, 0f, duration, 0f, 1f, tweenType);
		...
		{
			base.OnValidate();
			PLFeedbacksHelpers.MigrateCurve(AnimatePositionCurve, AnimatePositionTween, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimatePositionCurveX, AnimatePositionTweenX, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimatePositionCurveY, AnimatePositionTweenY, Owner);
			PLFeedbacksHelpers.MigrateCurve(AnimatePositionCurveZ, AnimatePositionTweenZ, Owner);
		}
	}
}
...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_AnimatorSpeed : PLF_Feedback 
	{
		...
		[PLFInspectorGroup("Speed", true, 14, true)]
		...
		[PLFEnumCondition("Mode", (int)SpeedModes.InstantThenReset, (int)SpeedModes.OverTime)]
		...
		[PLFEnumCondition("Mode", (int)SpeedModes.OverTime)]
		...
		protected virtual IEnumerator ChangeSpeedCo()
		{
			...
			else if (Mode == SpeedModes.OverTime)
			{
				...
				{
					float time = PLFeedbacksHelpers.Remap(FeedbackTime - _startedAt, 0f, Duration, 0f, 1f);
					float t = Curve.Evaluate(time);
					BoundAnimator.speed = Mathf.Max(0f, PLFeedbacksHelpers.Remap(t, 0f, 1f, _initialSpeed, newTargetSpeed));
					yield return null;
				}
				BoundAnimator.speed = _initialSpeed;	
				IsPlaying = false;

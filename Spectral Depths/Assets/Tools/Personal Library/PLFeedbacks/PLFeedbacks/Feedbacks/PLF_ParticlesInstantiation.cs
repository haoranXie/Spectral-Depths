...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_ParticlesInstantiation : PLF_Feedback
	{
		...
		public override Color FeedbackColor { get { return PLFeedbacksInspectorColors.ParticlesColor; } }
		...
		[PLFInspectorGroup("Particles Instantiation", true, 37, true)]
		...
		[PLFEnumCondition("Mode", (int)Modes.Pool)]
		...
		[PLFEnumCondition("Mode", (int)Modes.OnDemand)]
		...
		/// the duration for the player to consider. This won't impact your particle system, but is a way to communicate to the PLF Player the duration of this feedback. Usually you'll want it to match your actual particle system, and setting it can be useful to have this feedback work with holding pauses.
		[Tooltip("the duration for the player to consider. This won't impact your particle system, but is a way to communicate to the PLF Player the duration of this feedback. Usually you'll want it to match your actual particle system, and setting it can be useful to have this feedback work with holding pauses.")]
		public float DeclaredDuration = 0f;

		[PLFInspectorGroup("Position", true, 29)]
		...
		[PLFEnumCondition("PositionMode", (int)PositionModes.Transform)]
		...
		[PLFEnumCondition("PositionMode", (int)PositionModes.WorldPosition)]
		...
		[PLFEnumCondition("PositionMode", (int)PositionModes.Transform, (int)PositionModes.FeedbackPosition)]
		...
		[PLFInspectorGroup("Simulation Speed", true, 43, false)]
		...
		[PLFCondition("ForceSimulationSpeed", true)]
		...
		protected PLMiniObjectPooler _objectPooler; 
		...
		protected virtual void CreatePools(PLF_Player owner)
		{
			if (Mode != Modes.Pool)
			{
				return;
			...
			if (!_poolCreatedOrFound)
			{
				...
				_objectPooler = objectPoolGo.AddComponent<PLMiniObjectPooler>();
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (Mode == Modes.Pool)
			{
				if (_objectPooler != null)
				{
					...
					_instantiatedParticleSystem = _newGameObject.PLFGetComponentNoAlloc<ParticleSystem>();
					if (_instantiatedParticleSystem != null)
					{
						PositionParticleSystem(_instantiatedParticleSystem);

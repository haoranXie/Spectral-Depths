...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_Flicker : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Flicker", true, 61, true)]
		...
		protected override void CustomInitialization(PLF_Player owner)
		{
			...
			{
				if (Owner.gameObject.PLFGetComponentNoAlloc<Renderer>() != null)
				{
			...
			if (BoundRenderer == null)
			{
				Debug.LogWarning("[PLFeedbackFlicker] The flicker feedback on "+Owner.name+" doesn't have a bound renderer, it won't work. You need to specify a renderer to flicker in its inspector.");    
			}

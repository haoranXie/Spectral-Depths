...
namespace SpectralDepths.Feedbacks
{
	...
	public class PLF_ImageTextureOffset : PLF_Feedback
	{
		...
		[PLFInspectorGroup("Texture Scale", true, 63, true)]
		...
		[PLEnumCondition("MaterialPropertyType", (int)MaterialPropertyTypes.TextureID)]
		...
		[PLFInspectorGroup("Intensity", true, 65)]
		...
		{
			_newValue.x = PLFeedbacksHelpers.Remap(OffsetCurve.Evaluate(time), 0f, 1f, RemapZero.x, RemapOne.x);
			_newValue.y = PLFeedbacksHelpers.Remap(OffsetCurve.Evaluate(time), 0f, 1f, RemapZero.y, RemapOne.y);

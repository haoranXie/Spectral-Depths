...
namespace SpectralDepths.Feedbacks
{
	...
	public enum PLSequenceTrackStates { Idle, Down, Up }

	/// <summary>
	/// A class describing the contents of a sequence note, basically a timestamp and the ID to play at that timestamp
	/// </summary>
	[System.Serializable]
	public class PLSequenceNote
	{
		public float Timestamp;
		public int ID;

		public virtual PLSequenceNote Copy()
		{
			PLSequenceNote newNote = new PLSequenceNote();
			newNote.ID = this.ID;
	...
	public class PLSequenceTrack
	{
		...
		[PLFReadOnly]
		public PLSequenceTrackStates State = PLSequenceTrackStates.Idle;
		[HideInInspector]
		...
		public virtual void SetDefaults(int index)
		{
			...
			{
				ID = index;
				TrackColor = PLSequence.RandomSequenceColor();
				Key = KeyCode.Space;
				Active = true;
				State = PLSequenceTrackStates.Idle;
				Initialized = true;
			}            
		}
	}
	...
	/// </summary>
	[System.Serializable]
	public class PLSequenceList
	{
		public List<PLSequenceNote> Line;
	}

	/// <summary>
	/// This scriptable object holds "sequences", data used to record and play events in sequence
	/// PLSequences can be played by PLFeedbacks from their Timing section, by Sequencers and potentially other classes
	/// </summary>
	[CreateAssetMenu(menuName = "SpectralDepths/Sequencer/PLSequence")]
	public class PLSequence : ScriptableObject
	{
		[Header("Sequence")]
		...
		public PLSequenceList OriginalSequence;
		...
		public List<PLSequenceTrack> SequenceTracks;
		...
		public List<PLSequenceList> QuantizedSequence;
        
		[Space]
		[Header("Controls")]
		[PLFInspectorButton("RandomizeTrackColors")]
		public bool RandomizeTrackColorsButton;
        
		protected float[] _quantizedBeats; 
		protected List<PLSequenceNote> _deleteList;
		...
		/// <param name="p2"></param>
		/// <returns></returns>
		static int SortByTimestamp(PLSequenceNote p1, PLSequenceNote p2)
		{
		...
		/// </summary>
		public virtual void QuantizeSequenceToBPM(List<PLSequenceNote> baseSequence)
		{
			float sequenceLength = Length;
			float beatDuration = 60f / TargetBPM;
			int numberOfBeatsInSequence = (int)(sequenceLength / beatDuration);
			QuantizedSequence = new List<PLSequenceList>();
			_deleteList = new List<PLSequenceNote>();
			_deleteList.Clear();
			...
			{
				QuantizedSequence.Add(new PLSequenceList());
				QuantizedSequence[i].Line = new List<PLSequenceNote>();
				for (int j = 0; j < numberOfBeatsInSequence; j++)
				{
					...
					foreach (PLSequenceNote note in baseSequence)
					{
		...
		{
			foreach(PLSequenceTrack track in SequenceTracks)
			{

...
{
	/// <summary>
	/// This class lets you design a quantized PLSequence using a sequencer interface, and lets you play a quantized sequence, triggering events on beats if specified
	/// </summary>
	[AddComponentMenu("Spectral Depths/Feedbacks/Sequencing/PLSequencer")]
	public class PLSequencer : MonoBehaviour
	{
		[Header("Sequence")]
		...
		[HideInInspector]
		public PLSequence LastSequence;
		[HideInInspector]
		...
		public virtual void ApplySequencerLengthToSequence()
		{
			...
			{
				Sequence.QuantizedSequence = new List<PLSequenceList>();

				for (int trackIndex = 0; trackIndex < Sequence.SequenceTracks.Count; trackIndex++)
				{
					Sequence.QuantizedSequence.Add(new PLSequenceList());
					Sequence.QuantizedSequence[trackIndex].Line = new List<PLSequenceNote>();
					for (int i = 0; i < SequencerLength; i++)
					{
						PLSequenceNote note = new PLSequenceNote();
						note.ID = -1;

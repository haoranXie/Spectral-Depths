...
{
	public class PLMiniObjectPooler : MonoBehaviour
	{
		...
		protected PLMiniObjectPool _objectPool;
		protected const int _initialPoolsListCapacity = 5;
        
		static List<PLMiniObjectPool> _pools = new List<PLMiniObjectPool>(_initialPoolsListCapacity);

		/// <summary>
		/// Adds a pooler to the static list if needed
		/// </summary>
		/// <param name="pool"></param>
		public static void AddPool(PLMiniObjectPool pool)
		{
			if (_pools == null)
			{
				_pools = new List<PLMiniObjectPool>(_initialPoolsListCapacity);    
			}
		...
		/// </summary>
		/// <param name="pool"></param>
		public static void RemovePool(PLMiniObjectPool pool)
		{
			_pools?.Remove(pool);
		}
		...
		/// <returns></returns>
		public virtual PLMiniObjectPool ExistingPool(string poolName)
		{
			if (_pools == null)
			{
				_pools = new List<PLMiniObjectPool>(_initialPoolsListCapacity);    
			...
			if (_pools.Count == 0)
			{
				var pools = FindObjectsOfType<PLMiniObjectPool>();
				if (pools.Length > 0)
				{
					_pools.AddRange(pools);
				}
			}
			foreach (PLMiniObjectPool pool in _pools)
			{
		...
		protected virtual void CreateWaitingPool()
		{
			if (!MutualizeWaitingPools)
			{
				...
				_objectPool = this.gameObject.AddComponent<PLMiniObjectPool>();
				_objectPool.PooledGameObjects = new List<GameObject>();
				return;
			}
			else
			{
				PLMiniObjectPool waitingPool = ExistingPool(DetermineObjectPoolName(GameObjectToPool));
				...
				else
				{
					...
					_objectPool = newPool.AddComponent<PLMiniObjectPool>();
					_objectPool.PooledGameObjects = new List<GameObject>();
					AddPool(_objectPool);
				}
			}
	...
	public class PLMiniObjectPool : MonoBehaviour
	{
		[PLFReadOnly]

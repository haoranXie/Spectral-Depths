...
{
	public class PLShaker : PLMonoBehaviour
	{
		[PLInspectorGroup("Shaker Settings", true, 3)]
		...
		public virtual PLChannelData ChannelData => new PLChannelData(ChannelMode, Channel, PLChannelDefinition);
		...
		protected virtual float ShakeFloat(AnimationCurve curve, float remapMin, float remapMax, bool relativeIntensity, float initialValue)
		{
			...
			newValue = PLFeedbacksHelpers.Remap(curveValue, 0f, 1f, remapMin, remapMax);
		...
		/// <returns></returns>
		protected virtual bool CheckEventAllowed(PLChannelData channelData, bool useRange = false, float range = 0f, Vector3 eventOriginPosition = default(Vector3))
		{
		...
		public virtual float ComputeRangeIntensity(bool useRange, float rangeDistance, bool useRangeFalloff, AnimationCurve rangeFalloff, Vector2 remapRangeFalloff, Vector3 rangePosition)
		{
			...
			float normalizedDistance = PLMaths.Remap(distanceToCenter, 0f, rangeDistance, 0f, 1f);
			float curveValue = rangeFalloff.Evaluate(normalizedDistance);
			float newIntensity = PLMaths.Remap(curveValue, 0f, 1f, remapRangeFalloff.x, remapRangeFalloff.y);
			return newIntensity;
		}
	}
}
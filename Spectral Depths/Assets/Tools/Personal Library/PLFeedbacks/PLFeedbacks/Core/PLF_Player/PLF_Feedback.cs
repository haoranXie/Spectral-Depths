...
{
	[Serializable]
	public abstract class PLF_Feedback
	{
		#region Properties
		...
		[PLFInspectorGroup("Feedback Settings", true, 0, false, true)]
		...
		public string Label = "PLFeedback";
		...
		/// the PLChannel definition asset to use to broadcast this feedback. The shaker will have to reference that same PLChannel definition to receive events - to create a PLChannel,
		...
		/// use this color to customize the background color of the feedback in the PLF_Player's list
		[Tooltip("use this color to customize the background color of the feedback in the PLF_Player's list")]
		...
		public PLFeedbackTiming Timing;
		...
		public PLFeedbackTargetAcquisition AutomatedTargetAcquisition;
		
		[PLFInspectorGroup(_randomnessGroupName, true, 58, false, true)]
		...
		[Tooltip(
			"a random value (randomized between its x and y) by which to multiply the output of this feedback, if RandomizeOutput is true")]
		[PLFCondition("RandomizeOutput", true)]
		[PLFVector("Min", "Max")]
		...
		[Tooltip(
			"a random value (randomized between its x and y) by which to multiply the duration of this feedback, if RandomizeDuration is true")]
		[PLFCondition("RandomizeDuration", true)]
		[PLFVector("Min", "Max")]
		public Vector2 RandomDurationMultiplier = new Vector2(0.5f, 2f);

		[PLFInspectorGroup(_rangeGroupName, true, 47)]
		...
		[Tooltip("the values to remap the falloff curve's y axis' 0 and 1")] [PLFVector("Zero", "One")]
		...
		[HideInInspector] public PLF_Player Owner;
		...
		/// if this is true, this feedback will pause and wait until Resume() is called on its parent PLFeedbacks to resume execution
		...
		public virtual PLF_ReferenceHolder ForcedReferenceHolder { get; set; }
		...
		public virtual void CacheRequiresSetup()
		{
			...
			if (_requiresSetup && HasAutomatedTargetAcquisition && (AutomatedTargetAcquisition != null) && (AutomatedTargetAcquisition.Mode != PLFeedbackTargetAcquisition.Modes.None))
			{
		...
		/// if this is true, the feedback will be displayed in the PLF Player's list with a full color background, as opposed to just a small line on the left
		...
		public virtual string RequiredChannelText
		{
			get
			{
				if (ChannelMode == PLChannelModes.PLChannel)
				{
					if (PLChannelDefinition == null)
					{
						return "None";
					}

					return PLChannelDefinition.name;
				}
		...
		public PLChannelData ChannelData => _channelData.Set(ChannelMode, Channel, PLChannelDefinition);
		...
		protected PLChannelData _channelData;
		...
		public virtual void PreInitialization(PLF_Player owner, int index)
		{
			_channelData = new PLChannelData(ChannelMode, Channel, PLChannelDefinition);
		}
		...
		/// </summary>
		/// <param name="owner"></param>
		public virtual void Initialization(PLF_Player owner, int index)
		{
			if (Timing == null)
			{
				Timing = new PLFeedbackTiming();
			}
		...
		protected virtual void AutomateTargetAcquisitionInternal()
		{
			...
			{
				AutomatedTargetAcquisition = new PLFeedbackTargetAcquisition();
			}

			if (AutomatedTargetAcquisition.Mode == PLFeedbackTargetAcquisition.Modes.None)
			{
				return;
			}
		...
		{
			return PLFeedbackTargetAcquisition.FindAutomatedTargetGameObject(AutomatedTargetAcquisition, Owner, _indexInOwnerFeedbackList);
		}
		...
		{
			return PLFeedbackTargetAcquisition.FindAutomatedTarget<T>(AutomatedTargetAcquisition, Owner, _indexInOwnerFeedbackList);
		}
		...
		protected virtual IEnumerator SequenceCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			else
			{
				...
				{
					foreach (PLSequenceNote item in Timing.Sequence.OriginalSequence.Line)
					{
		...
		public virtual void SetSequence(PLSequence newSequence)
		{
		...
		/// <summary>
		/// Called when skipping to the end of PLF_Player, calls custom Skip on all feedbacks
		/// </summary>
		/// <param name="position"></param>
		/// <param name="feedbacksIntensity"></param>
		...
		/// <summary>
		/// Applies the host PLFeedbacks' time multiplier to this feedback
		/// </summary>
		...
		protected virtual IEnumerator WaitFor(float delay)
		{
			...
			{
				yield return PLFeedbacksCoroutine.WaitFor(delay);
			}
			else
			{
				yield return PLFeedbacksCoroutine.WaitForUnscaled(delay);
			}
		}
		...
		public virtual bool NormalPlayDirection
		{
			get
			{
				...
				{
					case PLFeedbackTiming.PlayDirections.FollowMMFeedbacksDirection:
						return (Owner.Direction == PLF_Player.Directions.TopToBottom);
					case PLFeedbackTiming.PlayDirections.AlwaysNormal:
						return true;
					case PLFeedbackTiming.PlayDirections.AlwaysRewind:
						return false;
					case PLFeedbackTiming.PlayDirections.OppositeMMFeedbacksDirection:
						return !(Owner.Direction == PLF_Player.Directions.TopToBottom);
				}
		...
		/// <summary>
		/// Returns true if this feedback should play in the current parent PLFeedbacks direction, according to its PLFeedbacksDirectionCondition setting
		/// </summary>
		public virtual bool ShouldPlayInThisSequenceDirection
		{
			get
			{
				...
				switch (Timing.PLFeedbacksDirectionCondition)
				{
					case PLFeedbackTiming.PLFeedbacksDirectionConditions.Always:
						return true;
					case PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenForwards:
						return (Owner.Direction == PLF_Player.Directions.TopToBottom);
					case PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenBackwards:
						return (Owner.Direction == PLF_Player.Directions.BottomToTop);
				}
		...
		/// </summary>
		/// <param name="owner"></param>
		protected virtual void CustomInitialization(PLF_Player owner) { }
		...
		/// <summary>
		/// Triggered when the host PLF Player gets disabled
		/// </summary>
		public virtual void OnDisable() { }

		/// <summary>
		/// Triggered when the host PLF Player gets selected, can be used to draw gizmos
		/// </summary>

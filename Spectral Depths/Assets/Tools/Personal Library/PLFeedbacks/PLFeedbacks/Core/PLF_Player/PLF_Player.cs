...
{
	[AddComponentMenu("Spectral Depths/Feedbacks/PLF Player")]
	[DisallowMultipleComponent] 
	public class PLF_Player : PLFeedbacks
	{
		#region PROPERTIES
        
		[SerializeReference]
		public List<PLF_Feedback> FeedbacksList;
		...
		protected override void Awake()
		{
			...
			if (AutoPlayOnEnable)
			{
				PLF_PlayerEnabler playerEnabler = GetComponent<PLF_PlayerEnabler>(); 
				if (playerEnabler == null)
				{
					playerEnabler = this.gameObject.AddComponent<PLF_PlayerEnabler>();
				}
		...
		protected virtual void InitializeFeedbackList()
		{
			...
			{
				FeedbacksList = new List<PLF_Feedback>();
			}
		}
		...
		protected override void OnEnable()
		{
			...
			{
				PLSetFeedbackRangeCenterEvent.Register(OnMMSetFeedbackRangeCenterEvent);	
			}
			foreach (PLF_Feedback feedback in FeedbacksList)
			{
		...
		{
			yield return PLFeedbacksCoroutine.WaitForFrames(framesAmount);
			PlayFeedbacks();
		}
		...
		/// <summary>
		/// When calling the legacy init method that used to specify an owner, we force the PLF Player init to run
		/// </summary>
		/// <param name="owner"></param>
		...
		public virtual bool IsAllowedToPlay(Vector3 position)
		{
			...
			// if all PLFeedbacks are disabled globally, we're not allowed to play
		...
		protected virtual IEnumerator FrameOnePlayCo(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
		{
			...
			_lastStartAt = _startTime;
			IsPlaying = true;
			yield return PLFeedbacksCoroutine.WaitForUnscaled(InitialDelay);
			PreparePlay(position, feedbacksIntensity, forceRevert);
		}
		...
		protected override IEnumerator PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
		{
			...
			while ((i >= 0) && (i < count))
			{
				...
				if ((FeedbacksList[i].LooperPause == true)
				    ...
				    && (FeedbacksList[i].ShouldPlayInThisSequenceDirection)
				    && (((FeedbacksList[i] as PLF_Looper).NumberOfLoopsLeft > 0) || (FeedbacksList[i] as PLF_Looper).InInfiniteLoop))
				{
					...
					bool loopAtLastPause = (FeedbacksList[i] as PLF_Looper).LoopAtLastPause;
					bool loopAtLastLoopStart = (FeedbacksList[i] as PLF_Looper).LoopAtLastLoopStart;
		...
		/// <summary>
		/// Adds the specified PLF_Feedback to the player
		/// </summary>
		/// <param name="newFeedback"></param>
		public virtual void AddFeedback(PLF_Feedback newFeedback)
		{
			InitializeFeedbackList();
		...
		/// <returns></returns>
		public new PLF_Feedback AddFeedback(System.Type feedbackType, bool add = true)
		{
			InitializeFeedbackList();
			...
			newFeedback.Owner = this;
			newFeedback.Timing = new PLFeedbackTiming();
			newFeedback.UniqueID = Guid.NewGuid().GetHashCode();
			if (add)
			{
				FeedbacksList.Add(newFeedback);	
		...
		public virtual float ComputeRangeIntensityMultiplier(Vector3 position)
		{
			...
			float normalizedDistance = PLFeedbacksHelpers.Remap(distanceToCenter, 0f, RangeDistance, 0f, 1f);
			float curveValue = RangeFalloff.Evaluate(normalizedDistance);
			float newIntensity = PLFeedbacksHelpers.Remap(curveValue, 0f, 1f, RemapRangeFalloff.x, RemapRangeFalloff.y);
			return newIntensity;
		}
		...
		/// <returns></returns>
		protected bool FeedbackCanPlay(PLF_Feedback feedback)
		{
		...
		/// <returns></returns>
		public virtual T GetFeedbackOfType<T>(AccessMethods method, int referenceIndex) where T:PLF_Feedback
		{
			_t = typeof(T);
		...
		/// <summary>
		/// Returns the first feedback of the searched type on this PLF_Player
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual T GetFeedbackOfType<T>() where T:PLF_Feedback
		{
			_t = typeof(T);
			foreach (PLF_Feedback feedback in FeedbacksList)
			{
		...
		/// <summary>
		/// Returns a list of all the feedbacks of the searched type on this PLF_Player
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual List<T> GetFeedbacksOfType<T>() where T:PLF_Feedback
		{
			_t = typeof(T);
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual T GetFeedbackOfType<T>(string searchedLabel) where T:PLF_Feedback
		{
			_t = typeof(T);
			foreach (PLF_Feedback feedback in FeedbacksList)
			{
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public virtual List<T> GetFeedbacksOfType<T>(string searchedLabel) where T:PLF_Feedback
		{
			_t = typeof(T);
			List<T> list = new List<T>();
			foreach (PLF_Feedback feedback in FeedbacksList)
		...
		/// <summary>
		/// When we get a PLSetFeedbackRangeCenterEvent, we set our new range center
		/// </summary>
		...
		protected override void OnDisable()
		{
			if (OnlyPlayIfWithinRange)
			{
				PLSetFeedbackRangeCenterEvent.Unregister(OnMMSetFeedbackRangeCenterEvent);	
			}
				...
				while ((i >= 0) && (i < FeedbacksList.Count) && (iterations < maxIterationsSafety))
				{
					...
					if ((FeedbacksList[i] != null) && FeedbacksList[i].Active && FeedbacksList[i].ShouldPlayInThisSequenceDirection)
					{
						...
						if (FeedbacksList[i].Pause != null)
						{
							...
							{
								intermediateTotal += (FeedbacksList[i] as PLF_Pause).PauseDuration;
								total += intermediateTotal;
								intermediateTotal = 0f;
							}
							else
							{
								currentPauseDelay += (FeedbacksList[i] as PLF_Pause).PauseDuration;
							}
							...
							if (FeedbacksList[i].LooperPause && ((FeedbacksList[i] as PLF_Looper).NumberOfLoops > 0))
							{
								...
								{
									lastLoopFoundAt = i;
									loopsLeft = (FeedbacksList[i] as PLF_Looper).NumberOfLoops - 1;
								}
								
								if ((FeedbacksList[i] as PLF_Looper).InfiniteLoop)
								{
									_cachedTotalDuration = 999f; 
									return;
								}

								if ((FeedbacksList[i] as PLF_Looper).LoopAtLastPause)
								{
									i = lastPauseFoundAt;
									total += intermediateTotal;
									intermediateTotal = 0f;
								...
								else if ((FeedbacksList[i] as PLF_Looper).LoopAtLastLoopStart)
								{
									i = lastLooperStart;
									total += intermediateTotal;
									intermediateTotal = 0f;
		...
		/// <summary>
		/// Draws gizmos, when the PLF_Player is selected, for all feedbacks that implement the method of the same name 
		/// </summary>

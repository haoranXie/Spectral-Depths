...
namespace SpectralDepths.Feedbacks
{
	...
	/// <summary>
	/// A class collecting delay, cooldown and repeat values, to be used to define the behaviour of each PLFeedback
	/// </summary>
	[System.Serializable]
	public class PLFeedbackTiming
	{
		/// the possible ways this feedback can play based on the host PLFeedbacks' directions
		public enum PLFeedbacksDirectionConditions { Always, OnlyWhenForwards, OnlyWhenBackwards };
		...
		/// whether to count this feedback in the parent PLFeedbacks(Player) total duration or not
		[Tooltip("whether to count this feedback in the parent PLFeedbacks(Player) total duration or not")]
		...
		[Header("Stop")]
		/// if this is true, this feedback will interrupt itself when Stop is called on its parent PLFeedbacks, otherwise it'll keep running
		[Tooltip("if this is true, this feedback will interrupt itself when Stop is called on its parent PLFeedbacks, otherwise it'll keep running")]
		...
		/// this defines how this feedback should play when the host PLFeedbacks is played :
		/// - always (default) : this feedback will always play
		/// - OnlyWhenForwards : this feedback will only play if the host PLFeedbacks is played in the top to bottom direction (forwards)
		/// - OnlyWhenBackwards : this feedback will only play if the host PLFeedbacks is played in the bottom to top direction (backwards)
		[Tooltip("this defines how this feedback should play when the host PLFeedbacks is played :" +
		         "- always (default) : this feedback will always play" +
		         "- OnlyWhenForwards : this feedback will only play if the host PLFeedbacks is played in the top to bottom direction (forwards)" +
		         "- OnlyWhenBackwards : this feedback will only play if the host PLFeedbacks is played in the bottom to top direction (backwards)")]
		public PLFeedbacksDirectionConditions PLFeedbacksDirectionCondition = PLFeedbacksDirectionConditions.Always;
		...
		/// - BasedOnMMFeedbacksDirection : will play normally when the host PLFeedbacks is played forwards, in rewind when it's played backwards
		/// - OppositeMMFeedbacksDirection : will play in rewind when the host PLFeedbacks is played forwards, and normally when played backwards
		/// - Always Normal : will always play normally, regardless of the direction of the host PLFeedbacks
		/// - Always Rewind : will always play in rewind, regardless of the direction of the host PLFeedbacks
		...
		[Header("Intensity")]
		/// if this is true, intensity will be constant, even if the parent PLFeedbacks is played at a lower intensity
		[Tooltip("if this is true, intensity will be constant, even if the parent PLFeedbacks is played at a lower intensity")]
		...
		[PLFCondition("UseIntensityInterval", true)]
		...
		[Header("Sequence")]
		/// A PLSequence to use to play these feedbacks on
		[Tooltip("A PLSequence to use to play these feedbacks on")]
		public PLSequence Sequence;
		/// The PLSequence's TrackID to consider
		[Tooltip("The PLSequence's TrackID to consider")]

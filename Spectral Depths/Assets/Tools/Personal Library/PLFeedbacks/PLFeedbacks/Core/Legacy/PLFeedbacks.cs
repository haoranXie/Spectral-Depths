...
{
	/// <summary>
	/// A collection of PLFeedback, meant to be played altogether.
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Feedbacks/PLFeedbacks")]
	public class PLFeedbacks : MonoBehaviour
	{
		/// the possible directions PLFeedbacks can be played
		...
		/// a list of PLFeedback to trigger
		public List<PLFeedback> Feedbacks = new List<PLFeedback>();
		...
		/// Otherwise, you can have this component initialize itself at Awake or Start, and in this case the owner will be the PLFeedbacks itself
		...
		[Tooltip("the chosen initialization modes. If you use Script, you'll have to initialize manually by calling the " +
		         ...
		         "itself at Awake or Start, and in this case the owner will be the PLFeedbacks itself")]
		...
		/// whether or not this PLFeedbacks should invert its direction when all feedbacks have played
		[Tooltip("whether or not this PLFeedbacks should invert its direction when all feedbacks have played")]
		...
		[PLFCondition("ForceTimescaleMode", true)]
		...
		[PLCondition("RandomizeDuration", true)]
		...
		[PLFCondition("UseRangeFalloff", true)]
		...
		/// whether or not to ignore PLSetFeedbackRangeCenterEvent, used to set the RangeCenter from anywhere
		[Tooltip("whether or not to ignore PLSetFeedbackRangeCenterEvent, used to set the RangeCenter from anywhere")]
		public bool IgnoreRangeEvents = false;

		/// a duration, in seconds, during which triggering a new play of this PLFeedbacks after it's been played once will be impossible
		[Tooltip("a duration, in seconds, during which triggering a new play of this PLFeedbacks after it's been played once will be impossible")]
		public float CooldownDuration = 0f;
		/// a duration, in seconds, to delay the start of this PLFeedbacks' contents play
		[Tooltip("a duration, in seconds, to delay the start of this PLFeedbacks' contents play")]
		...
		/// a number of UnityEvents that can be triggered at the various stages of this PLFeedbacks 
		[Tooltip("a number of UnityEvents that can be triggered at the various stages of this PLFeedbacks")] 
		public PLFeedbacksEvents Events;
		...
		[HideInInspector]
		/// whether or not this PLFeedbacks is in debug mode
		public bool DebugActive = false;
		/// whether or not this PLFeedbacks is playing right now - meaning it hasn't been stopped yet.
		/// if you don't stop your PLFeedbacks it'll remain true of course
		public bool IsPlaying { get; protected set; }
		/// if this PLFeedbacks is playing the time since it started playing
		public float ElapsedTime => IsPlaying ? GetTime() - _lastStartAt : 0f;
		/// the amount of times this PLFeedbacks has been played
		public int TimesPlayed { get; protected set; }
		/// whether or not the execution of this PLFeedbacks' sequence is being prevented and waiting for a Resume() call
		public bool InScriptDrivenPause { get; set; }
		/// true if this PLFeedbacks contains at least one loop
		...
		/// The total duration (in seconds) of all the active feedbacks in this PLFeedbacks
		public virtual float TotalDuration
		{
			get
			{
				float total = 0f;
				foreach (PLFeedback feedback in Feedbacks)
				{
		...
		{
			// if our PLFeedbacks is in AutoPlayOnEnable mode, we add a little helper to it that will re-enable it if needed if the parent game object gets turned off and on again
			if (AutoPlayOnEnable)
			{
				PLFeedbacksEnabler enabler = GetComponent<PLFeedbacksEnabler>(); 
				if (enabler == null)
				{
					enabler = this.gameObject.AddComponent<PLFeedbacksEnabler>();
				}
		...
		/// <summary>
		/// Initializes the PLFeedbacks, setting this PLFeedbacks as the owner
		/// </summary>
		...
		{
			if ((SafeMode == PLFeedbacks.SafeModes.RuntimeOnly) || (SafeMode == PLFeedbacks.SafeModes.Full))
			{
				AutoRepair();
			}
		...
		/// <summary>
		/// Plays all feedbacks using the PLFeedbacks' position as reference, and no attenuation
		/// </summary>
		...
		/// <summary>
		/// Plays all feedbacks using the PLFeedbacks' position as reference, and no attenuation, and in reverse (from bottom to top)
		/// </summary>
		...
		/// <summary>
		/// Plays all feedbacks in the sequence, but only if this PLFeedbacks is playing in reverse order
		/// </summary>
		...
		/// <summary>
		/// Plays all feedbacks in the sequence, but only if this PLFeedbacks is playing in normal order
		/// </summary>
		...
		/// <summary>
		/// A public coroutine you can call externally when you want to yield in a coroutine of yours until the PLFeedbacks has stopped playing
		/// typically : yield return myFeedback.PlayFeedbacksCoroutine(this.transform.position, 1.0f, false);
		/// </summary>
		/// <param name="position">The position at which the PLFeedbacks should play</param>
		/// <param name="feedbacksIntensity">The intensity of the feedback</param>
		/// <param name="forceRevert">Whether or not the PLFeedbacks should play in reverse or not</param>
		/// <returns></returns>
		...
		protected virtual void PlayFeedbacksInternal(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
		{
			...
			// if all PLFeedbacks are disabled globally, we stop and don't play
		...
		{
			IsPlaying = true;
			yield return PLFeedbacksCoroutine.WaitFor(InitialDelay);
			PreparePlay(position, feedbacksIntensity, forceRevert);
		}
		...
		protected virtual IEnumerator PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
		{
			...
			while ((i >= 0) && (i < Feedbacks.Count))
			{
				...
				if ((Feedbacks[i].LooperPause == true)
				    && (Feedbacks[i].Active)
				    && (Feedbacks[i].ShouldPlayInThisSequenceDirection)
				    && (((Feedbacks[i] as PLFeedbackLooper).NumberOfLoopsLeft > 0) || (Feedbacks[i] as PLFeedbackLooper).InInfiniteLoop))
				{
					// we determine the index we should start again at
					bool loopAtLastPause = (Feedbacks[i] as PLFeedbackLooper).LoopAtLastPause;
					bool loopAtLastLoopStart = (Feedbacks[i] as PLFeedbackLooper).LoopAtLastLoopStart;
		...
		/// <summary>
		/// Changes the direction of this PLFeedbacks
		/// </summary>
		...
		public virtual PLFeedback AddFeedback(System.Type feedbackType, bool add = true)
		{
			PLFeedback newFeedback;
            
			#if UNITY_EDITOR
			if (!Application.isPlaying)
			{
				newFeedback = Undo.AddComponent(this.gameObject, feedbackType) as PLFeedback;
			}
			else
			{
				newFeedback = this.gameObject.AddComponent(feedbackType) as PLFeedback;
			}
			#else 
                newFeedback = this.gameObject.AddComponent(feedbackType) as PLFeedback;
			#endif
		...
		/// <summary>
		/// Checks whether or not this PLFeedbacks contains one or more looper feedbacks
		/// </summary>
		...
		/// <summary>
		/// This will return true if the conditions defined in the specified feedback's Timing section allow it to play in the current play direction of this PLFeedbacks
		/// </summary>
		/// <param name="feedback"></param>
		/// <returns></returns>
		protected bool FeedbackCanPlay(PLFeedback feedback)
		{
			if (feedback == null)
			{
				return false;
			...
			if (feedback.Timing.PLFeedbacksDirectionCondition == PLFeedbackTiming.PLFeedbacksDirectionConditions.Always)
			{
				return true;
			}
			else if (((Direction == Directions.TopToBottom) && (feedback.Timing.PLFeedbacksDirectionCondition == PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenForwards))
			         || ((Direction == Directions.BottomToTop) && (feedback.Timing.PLFeedbacksDirectionCondition == PLFeedbackTiming.PLFeedbacksDirectionConditions.OnlyWhenBackwards)))
			{
				return true;
			}
			return false;
		...
		/// <summary>
		/// Readies the PLFeedbacks to revert direction on the next play
		/// </summary>
		...
		public virtual void AutoRepair()
		{
			...
			{
				if (component is PLFeedback)
				{
					bool found = false;
					for (int i = 0; i < Feedbacks.Count; i++)
					{
						if (Feedbacks[i] == (PLFeedback)component)
						{
							found = true;
							break;
						}
					...
					if (!found)
					{
						Feedbacks.Add((PLFeedback)component);
					}
				}
			}
		} 
		...
		/// <summary>
		/// On Destroy, removes all feedbacks from this PLFeedbacks to avoid any leftovers
		/// </summary>

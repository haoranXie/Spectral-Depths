...
{
	[CustomEditor(typeof(PLRadioSignalGenerator), true)]
	[CanEditMultipleObjects]
	public class PLRadioSignalGeneratorEditor : PLRadioSignalEditor
	{
		...
		protected Color _spectrumColor = PLColors.Aqua;
		protected Color _spectrumBoxColor = PLColors.AliceBlue;
		...
		protected override void OnEnable()
		{
			...
			_list = new PLReorderableList(serializedObject.FindProperty("SignalList"));
		...
		private void OnListAdd(PLReorderableList list)
		{
		...
		protected virtual void DrawRawSpectrum()
		{
			...
			_normalizedTime = PLMaths.Remap((float)_signalTime, 0f, _duration.floatValue, 0f, 1f);
			...
			if (_animatedPreview.boolValue)
			{
				...
				zeroX = maxX - PLMaths.Remap(_normalizedTime, 0f, 1f, _spectrumBoxPosition.x + _externalMargin, _spectrumBoxPosition.x + _spectrumBoxSize.x);
				oneX = zeroX - 10;
			}
			...
			if (_duration.floatValue > 0f)
			{
				...
				if (Application.isPlaying)
				{
					boxSpectrumValue = PLMaths.Remap(_radioSignal.GraphValue(_driverTime.floatValue), 0f, 1f, 0f, _spectrumBoxSize.y);
				}
				else
				{
					boxSpectrumValue = PLMaths.Remap(_radioSignal.GraphValue(_normalizedTime), 0f, 1f, 0f, _spectrumBoxSize.y);
				}
			...
			if (Application.isPlaying && !_animatedPreview.boolValue)
			{
				_rect.x = _spectrumBoxPosition.x 
				          + PLMaths.Remap(_driverTime.floatValue, 0f, 1f, 0f, _spectrumBoxSize.x);
			...
			{
				float xPosition = _spectrumBoxPosition.x + _externalMargin + PLMaths.Remap(i, 0, _spectrumPointsCount, 0f, _spectrumBoxSize.x - _externalMargin * 2);
				...
				float spectrumValue = PLMaths.Remap(_radioSignal.GraphValue(time), 0f, 1f, 0f, _spectrumBoxSize.y);
				float spectrumValuePrevious = PLMaths.Remap(_radioSignal.GraphValue(timePrevious), 0f, 1f, 0f, _spectrumBoxSize.y);

...
namespace SpectralDepths.Tools
{
	...
	public class Beat
	{
		...
		[PLEnumCondition("Mode", (int)Modes.Raw, (int)Modes.Normalized, (int)Modes.BufferedRaw, (int)Modes.BufferedNormalized)]
		public Color BeatColor = Color.cyan;
		public int BandID = 0;
		public float Threshold = 0.5f;
		public float MinimumTimeBetweenBeats = 0.25f;

		[PLEnumCondition("BeatValueMode", (int)BeatValueModes.Remapped)]
		...
		public void InitializeIfNeeded(int id, int bandID)
		{
			if (!_initialized)
			{
				...
				Name = "Beat " + id;
				BeatColor = PLColors.RandomColor();
				BandID = bandID;
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Audio/PLAudioAnalyzer")]
	public class PLAudioAnalyzer : MonoBehaviour
	{
		public enum Modes { Global, AudioSource, Microphone }

		[Header("Source")]
		[PLInformation("This component lets you pick an audio source (either global : the whole scene's audio, a unique source, or the " +
		               "microphone), and will cut it into chunks that you can then use to emit beat events, that other objects can consume and act upon. " +
		               "The sample interval is the frequency at which sound will be analyzed, the amount of spectrum samples will determine the " +
		               "accuracy of the sampling, the window defines the method used to reduce leakage, and the number of bands " +
		               "will determine in how many bands you want to cut the sound. The more bands, the more levers you'll have to play with afterwards." +
		               "In general, for all of these settings, higher values mean better quality and lower performance. The buffer speed determines how " +
		               "fast buffered band levels readjust.", SpectralDepths.Tools.PLInformationAttribute.InformationType.Info, false)]
		[PLReadOnlyWhenPlaying]
		public Modes Mode = Modes.Global;
		[PLEnumCondition("Mode", (int)Modes.AudioSource)]
		[PLReadOnlyWhenPlaying]
		public AudioSource TargetAudioSource;
		[PLEnumCondition("Mode", (int)Modes.Microphone)]
		public int MicrophoneID = 0;

		[Header("Sampling")]
		[PLReadOnlyWhenPlaying]
		public float SampleInterval = 0.02f;
		[PLDropdown(2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192)]
		[PLReadOnlyWhenPlaying]
		...
		public virtual void Initialization()
		{
			...
			{
				Debug.LogError(this.name + " : this PLAudioAnalyzer needs a target audio source to operate.");
				return;
			}
		...
		protected virtual void OnBeat(Beat beat, float rawValue)
		{
			...
			PLBeatEvent.Trigger(beat.Name, beat.CurrentValue);

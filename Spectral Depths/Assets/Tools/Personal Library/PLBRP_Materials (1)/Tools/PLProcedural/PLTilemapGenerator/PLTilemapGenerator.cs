...
namespace SpectralDepths.Tools 
{
	...
	/// </summary>
	[ExecuteAlways]  
	public class PLTilemapGenerator : MonoBehaviour
	{
		[Header("Grid")] 
		...
		public PLTilemapGeneratorLayerList Layers;
		...
		public PLTweenType SlowRenderTweenType = new PLTweenType(PLTween.PLTweenCurve.EaseInOutCubic);
		...
		public virtual void Generate()
		{
			...
			foreach (PLTilemapGeneratorLayer layer in Layers)
			{
				GenerateLayer(layer);   
			}
		}
		...
		void Reset()
		{
			Layers = new PLTilemapGeneratorLayerList(){
				new PLTilemapGeneratorLayer()
			};
		}
		...
		protected virtual void GenerateLayer(PLTilemapGeneratorLayer layer)
		{
			if (!layer.Active)
			{
				return;
			...
			switch (layer.GenerateMethod)
			{
				case GenerateMethods.Full:
					_grid = PLGridGeneratorFull.Generate(width, height, layer.FullGenerationFilled);
					layer.Grid = _grid;
					break;
				case GenerateMethods.Perlin:
					_grid = PLGridGeneratorPerlinNoise.Generate(width, height, seedFloat);
					layer.Grid = _grid;
					break;
				case GenerateMethods.PerlinGround:
					_grid = PLGridGeneratorPerlinNoiseGround.Generate(width, height, seedFloat);
					layer.Grid = _grid;
					break;
				case GenerateMethods.Random:
					_grid = PLGridGeneratorRandom.Generate(width, height, seed, layer.RandomFillPercentage);
					layer.Grid = _grid;
					break;
				case GenerateMethods.RandomWalk:
					_grid = PLGridGeneratorRandomWalk.Generate(width, height, seed, layer.RandomWalkPercent, layer.RandomWalkStartingPoint, layer.RandomWalkMaxIterations);
					layer.Grid = _grid;
					break;
				case GenerateMethods.RandomWalkAvoider:

					int[,] obstacleGrid = PLGridGenerator.TilemapToGrid(layer.RandomWalkAvoiderObstaclesTilemap, width, height);
					_grid = PLGridGeneratorRandomWalkAvoider.Generate(width, height, seed, layer.RandomWalkAvoiderPercent, layer.RandomWalkAvoiderStartingPoint, obstacleGrid, layer.RandomWalkAvoiderObstaclesDistance, layer.RandomWalkAvoiderMaxIterations);
					layer.Grid = _grid;
					break;
				case GenerateMethods.RandomWalkGround:
					_grid = PLGridGeneratorRandomWalkGround.Generate(width, height, seed, 
				...
				case GenerateMethods.Path:
					_grid = PLGridGeneratorPath.Generate(width, height, seed, layer.PathDirection, layer.PathStartPosition, layer.PathMinWidth,
			...
			if (layer.Smooth) { _grid = PLGridGenerator.SmoothenGrid(_grid); }
			if (layer.InvertGrid) { _grid = PLGridGenerator.InvertGrid(_grid); }

			_grid = PLGridGenerator.BindGrid(_grid, layer.BoundsTop, layer.BoundsBottom, layer.BoundsLeft, layer.BoundsRight);
			_grid = PLGridGenerator.ApplySafeSpots(_grid, layer.SafeSpots);
		...
		/// </summary>
		/// <param name="layer"></param>
		async static void DelayedCopy(PLTilemapGeneratorLayer layer)
		{
			await Task.Delay(500);
			PLTilemapShadow.Copy(layer.CopyTilemap, layer.TargetTilemap);
		}
		...
		/// </summary>
		/// <param name="layer"></param>
		protected virtual void RenderGrid(PLTilemapGeneratorLayer layer)
		{
			PLTilemapGridRenderer.RenderGrid(_grid, layer, SlowRender, SlowRenderDuration, SlowRenderTweenType,this);
		}

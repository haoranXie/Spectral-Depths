...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[Serializable]
	[AddComponentMenu("Spectral Depths/Tools/Cinemachine/PLGyroCam")]
	public class PLGyroCam
	{
		#if PL_CINEMACHINE
	...
	/// </summary>
	public class PLGyroParallax : PLGyroscope
	{
		[Header("Cameras")]
		/// the list of cameras to move as the gyro moves
		public List<PLGyroCam> Cams;
		...
		protected virtual void MoveCameras()
		{
			foreach (PLGyroCam cam in Cams)
			{
				...
				if (gyroGravity.x > 0)
				{
					newX = PLMaths.Remap(LerpedCalibratedGyroscopeGravity.x, 0.5f, 0, cam.MinRotation.x, 0);
				}
				if (gyroGravity.x < 0)
				{
					newX = PLMaths.Remap(LerpedCalibratedGyroscopeGravity.x, 0, -.5f, 0, cam.MaxRotation.x);
				}
				if (gyroGravity.y > 0)
				{
					newY = PLMaths.Remap(LerpedCalibratedGyroscopeGravity.y, 0.5f, 0, cam.MinRotation.y, 0f);
				}
				if (gyroGravity.y < 0)
				{
					newY = PLMaths.Remap(LerpedCalibratedGyroscopeGravity.y, 0f, -0.5f, 0f, cam.MaxRotation.y);
				}

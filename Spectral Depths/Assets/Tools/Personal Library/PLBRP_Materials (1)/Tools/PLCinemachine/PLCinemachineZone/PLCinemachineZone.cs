...
namespace SpectralDepths.Tools
{
	...
	[AddComponentMenu("")]
	[ExecuteAlways]
	public abstract class PLCinemachineZone : MonoBehaviour
	{
		...
		[PLEnumCondition("Mode", (int)Modes.Priority)]
		...
		[PLInspectorButton("ManualSetupConfiner")]
		...
		protected virtual void Initialization()
		{
			...
			if (VirtualCamera == null)
			{
				Debug.LogWarning("[PLCinemachineZone2D] " + this.name + " : no virtual camera is attached to this zone. Set one in its inspector.");
			}
		...
		protected virtual IEnumerator EnableCamera(bool state, int frames)
		{
			...
			if (frames > 0)
			{
				yield return PLCoroutine.WaitForFrames(frames);    
			}
		...
		{
			GizmosColor = PLColors.RandomColor();
			GizmosColor.a = 0.2f;
		}

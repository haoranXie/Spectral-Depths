...
{	
	...
	/// </summary>
	public class PLMultipleObjectPoolerObject
	{
	...
	/// </summary>
	public enum PLPoolingMethods { OriginalOrder, OriginalOrderSequential, RandomBetweenObjects, RandomPoolSizeBased }

	/// <summary>
	/// This class allows you to have a pool of various objects to pool from.
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Object Pool/PLMultipleObjectPooler")]
	public class PLMultipleObjectPooler : PLObjectPooler
	{
		/// the list of objects to pool
		public List<PLMultipleObjectPoolerObject> Pool;
		[PLInformation("A MultipleObjectPooler is a reserve of objects, to be used by a Spawner. When asked, it will return an object from the pool (ideally an inactive one) chosen based on the pooling method you've chosen.\n- OriginalOrder will spawn objects in the order you've set them in the inspector (from top to bottom)\n- OriginalOrderSequential will do the same, but will empty each pool before moving to the next object\n- RandomBetweenObjects will pick one object from the pool, at random, but ignoring its pool size, each object has equal chances to get picked\n- PoolSizeBased randomly choses one object from the pool, based on its pool size probability (the larger the pool size, the higher the chances it'll get picked)'...",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		/// the chosen pooling method
		public PLPoolingMethods PoolingMethod = PLPoolingMethods.RandomPoolSizeBased;
		[PLInformation("If you set CanPoolSameObjectTwice to false, the Pooler will try to prevent the same object from being pooled twice to avoid repetition. This will only affect random pooling methods, not ordered pooling.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		/// a unique name that should match on all PLMultipleObjectPoolers you want to use together
		[PLCondition("MutualizeWaitingPools", true)]
		public string MutualizedPoolName = "";
		
		public List<PLMultipleObjectPooler> Owner { get; set; }
		...
		public override void FillObjectPool()
		{
			...
			{
				case PLPoolingMethods.OriginalOrder:
					...
					break;
				case PLPoolingMethods.OriginalOrderSequential:
					// we store our poolsizes in a temp array so it doesn't impact the inspector
					foreach (PLMultipleObjectPoolerObject pooledGameObject in Pool)
					{
		...
		public override GameObject GetPooledGameObject()
		{
			...
			switch (PoolingMethod)
			{
				case PLPoolingMethods.OriginalOrder:
					...
					break;
				case PLPoolingMethods.RandomPoolSizeBased:
					pooledGameObject =  GetPooledGameObjectPoolSizeBased();
					break;
				case PLPoolingMethods.RandomBetweenObjects:
		...
		protected virtual GameObject GetPooledGameObjectOriginalOrder()
		{
			...
			PLMultipleObjectPoolerObject searchedObject = GetPoolObject(Pool[_currentIndex].GameObjectToPool);
		...
		protected virtual void OrderSequentialResetCounter(PLMultipleObjectPoolerObject searchedObject)
		{
		...
		protected virtual GameObject GetPooledGameObjectPoolSizeBased()
		{
			...
			{	
				...
				else
				{
					...
					PLMultipleObjectPoolerObject searchedObject = GetPoolObject(_objectPool.PooledGameObjects[randomIndex].gameObject);
		...
		protected virtual PLMultipleObjectPoolerObject GetPoolObject(GameObject testedObject)
		{
			...
			int i=0;
			foreach(PLMultipleObjectPoolerObject poolerObject in Pool)
			{
		...
		{
			PLMultipleObjectPoolerObject searchedObject = GetPoolObject(testedObject);
			if (searchedObject != null)
			{

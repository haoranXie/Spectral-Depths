...
namespace SpectralDepths.Tools
{
	...
	public class PLSpawnAroundProperties
	{
		...
		[PLEnumCondition("Shape", (int)PLSpawnAroundShapes.Sphere)]
		...
		[PLEnumCondition("Shape", (int)PLSpawnAroundShapes.Cube)]
		...
		[PLCondition("UseNormalAxisOffsetCurve",true)]
	...
	/// instantiate objects  
	/// </summary>
	public static class PLSpawnAround 
	{
		public static void ApplySpawnAroundProperties(GameObject instantiatedObj, PLSpawnAroundProperties props, Vector3 origin)
		{            
		...
		/// <returns></returns>
		public static Vector3 SpawnAroundPosition(PLSpawnAroundProperties props, Vector3 origin)
		{
			// we get the position of the object based on the defined plane and distance
			Vector3 newPosition;
			if (props.Shape == PLSpawnAroundProperties.PLSpawnAroundShapes.Sphere)
			{
			...
			if (props.UseNormalAxisOffsetCurve)
			{
				...
				if (randomOffset != 0)
				{
					...
					{
						normalizedOffset = PLMaths.Remap(randomOffset, props.MinimumNormalAxisOffset, props.MaximumNormalAxisOffset, 1f, 0f);
					}
					else
					{
						normalizedOffset = PLMaths.Remap(randomOffset, props.MinimumNormalAxisOffset, props.MaximumNormalAxisOffset, 0f, 1f);
					}
				}

				float offset = props.NormalOffsetCurve.Evaluate(normalizedOffset);
				offset = PLMaths.Remap(offset, 0f, 1f, props.NormalOffsetCurveRemapZero, props.NormalOffsetCurveRemapOne);
		...
		/// <param name="props"></param>
		/// <returns></returns>
		public static Vector3 SpawnAroundScale(PLSpawnAroundProperties props)
		{
			return PLMaths.RandomVector3(props.MinimumScale, props.MaximumScale);
		}
		...
		/// <param name="props"></param>
		/// <returns></returns>
		public static Quaternion SpawnAroundRotation(PLSpawnAroundProperties props)
		{
			return Quaternion.Euler(PLMaths.RandomVector3(props.MinimumRotation, props.MaximumRotation));
		}
		...
		public static void DrawGizmos(PLSpawnAroundProperties props, Vector3 origin, int quantity, float size, Color gizmosColor)
		{

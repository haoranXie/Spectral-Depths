...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Movement/PLPathMovement")]
	public class PLPathMovement : MonoBehaviour 
	{
		...
		[Header("Movement")]
		[PLInformation("Set the <b>speed</b> at which the path will be crawled, and if the movement should be constant or eased.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		/// the movement speed
		...
		[Header("Settings")]
		[PLInformation("The <b>MinDistanceToGoal</b> is used to check if we've (almost) reached a point in the Path. The 2 other settings here are for debug only, don't change them.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		public virtual void MoveAlongThePath()
		{
			switch (AccelerationType)
			{
				...
				case PossibleAccelerationType.AnimationCurve:
					...
					float remappedDistance = 1 - PLMaths.Remap (_distanceToNextPoint, 0f, distanceBetweenPoints, 0f, 1f);
		...
		{	
			...
			for (int i=0;i<PathElements.Count;i++)
			{
				...
				PLDebug.DrawGizmoPoint(PointPosition(i),0.2f,Color.green);
			...
			{
				PLDebug.DrawGizmoPoint(PointPosition(_currentPoint.Current), 0.2f, Color.blue);
				PLDebug.DrawGizmoPoint(PointPosition(_previousPoint),0.2f,Color.red);
			}
			#endif
		}

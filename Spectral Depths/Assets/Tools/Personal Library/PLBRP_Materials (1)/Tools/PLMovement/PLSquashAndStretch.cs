...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Movement/PLSquashAndStretch")]
	public class PLSquashAndStretch : MonoBehaviour
	{
		public enum Timescales { Regular, Unscaled }
		public enum Modes { Rigidbody, Rigidbody2D, Position }

		[PLInformation("This component will apply squash and stretch based on velocity (either position based or computed from a Rigidbody. It has to be put on an intermediary level in the hierarchy, between the logic (top level) and the model (bottom level).", PLInformationAttribute.InformationType.Info, false)]
		...
		protected virtual void ComputeVelocityAndDirection()
		{
			...
			RemappedVelocity = PLMaths.Remap(VelocityMagnitude, 0f, MaximumVelocity, 0f, 1f);
			...
			if (AutoSquashOnStop)
			{
				...
				else if (_movementStarted)
				{
					...
					_squashing = true;
					float duration = PLMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashDuration.x, SquashDuration.y);
					float intensity = PLMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashIntensity.x, SquashIntensity.y);
		...
		protected virtual void ComputeNewLocalScale()
		{
			if (_squashing)
			{
				float elapsed = PLMaths.Remap(TimescaleTime - _squashStartedAt, 0f, _squashDuration, 0f, 1f);
			...
			if (Spring)
			{
				PLMaths.Spring(ref _springScale, _newLocalScale, ref _springVelocity, SpringDamping, SpringFrequency, SpringSpeed, Time.deltaTime);

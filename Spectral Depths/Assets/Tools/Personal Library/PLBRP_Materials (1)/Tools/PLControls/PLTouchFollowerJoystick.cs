...
namespace SpectralDepths.Tools
{
	...
	/// Note that this component extends the PLTouchJoystick class so you don't need to add another joystick to it. It's both the detection zone and the stick itself.
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Controls/PLTouchFollowerJoystick")]
	public class PLTouchFollowerJoystick : PLTouchJoystick
	{
		[PLInspectorGroup("Follower Joystick", true, 23)]
		...
		[PLCondition("InterpolateFollowMovement", true)]
		...
		[PLCondition("SpringFollowInterpolation", true)]
		...
		[PLInspectorGroup("Background Constraints", true, 24)]
		...
		protected virtual void HandleMovementInterpolation()
		{
			...
			if (SpringFollowInterpolation)
			{
				...
				PLMaths.Spring(ref _newBackgroundPosition, _backgroundPositionTarget, ref _springVelocity, SpringDamping, SpringFrequency, InterpolateFollowMovementSpeed, Time.unscaledDeltaTime);
				BackgroundCanvasGroup.transform.position = _newBackgroundPosition;
			}
			else
			{
				BackgroundCanvasGroup.transform.position = PLMaths.Lerp(BackgroundCanvasGroup.transform.position, _backgroundPositionTarget, InterpolateFollowMovementSpeed, Time.unscaledDeltaTime);	
			}
		}
		...
		protected override void OnDrawGizmos()
		{
			...
			if (BackgroundConstraintRectTransform != null)
			{
				...
				PLDebug.DrawGizmoPoint(_innerRectTransformBottomLeft, Color.cyan, gizmoSize);
				PLDebug.DrawGizmoPoint(_innerRectTransformTopLeft, Color.cyan, gizmoSize);
				PLDebug.DrawGizmoPoint(_innerRectTransformTopRight, Color.cyan, gizmoSize);
				PLDebug.DrawGizmoPoint(_innerRectTransformBottomRight, Color.cyan, gizmoSize);
			}
		}
		#endif
	}

...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	public class PLExecutionOrderAttribute : Attribute
	{
		#if UNITY_EDITOR
		/// the execution order you want for the class this attribute is applied to 
		public int ExecutionOrder = 0;

		protected static Dictionary<MonoScript, PLExecutionOrderAttribute> _monoScripts;
		...
		public PLExecutionOrderAttribute(int newExecutionOrder)
		{
		...
		{
			_monoScripts = new Dictionary<MonoScript, PLExecutionOrderAttribute>();
			_executionOrderAttributeType = typeof(PLExecutionOrderAttribute);
		...
		protected static void FindExecutionOrderAttributes()
		{
			foreach (Type assemblyType in _assemblyTypes)
			{
				...
				PLExecutionOrderAttribute attribute = attributes[0] as PLExecutionOrderAttribute;
				...
				{
					Debug.LogError("PLTools' ExecutionOrderAttribute : Can't change "+ assemblyType.Name + "'s execution order");
					return;
				}
		...
		protected static bool ExecutionOrderHasChanged()
		{
			...
			foreach (KeyValuePair<MonoScript, PLExecutionOrderAttribute> monoScript in _monoScripts)
			{

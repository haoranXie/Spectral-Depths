...
namespace SpectralDepths.Tools
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Tools/Sprites/PLAutoOrderInLayer")]
	public class PLAutoOrderInLayer : MonoBehaviour 
	{
		static int CurrentMaxCharacterOrderInLayer = 0;

		[Header("Global Counter")]
		[PLInformation("Add this component to an object with a sprite renderer, and it'll give it a new order in layer based on the settings defined here. First is the global counter increment, or how much you'd like to increment the layer order between two objects on that same layer.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		/// the number by which to increment each new object's order in layer
		public int GlobalCounterIncrement = 5;

		[Header("Parent")]
		[PLInformation("You can also decide to determine the new layer order based on the parent sprite's order (it'll have to be on the same layer).",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		[Header("Children")]
		[PLInformation("And here you can decide to apply your new layer order to all children.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]

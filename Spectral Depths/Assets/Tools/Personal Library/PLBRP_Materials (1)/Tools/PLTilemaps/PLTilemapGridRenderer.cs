...
{
	public class PLTilemapGridRenderer 
	{
		/// <summary>
		...
		public static void RenderGrid(int[,] grid, PLTilemapGeneratorLayer layer, bool slowRender = false, float slowRenderDuration = 1f, 
			PLTweenType slowRenderTweenType = null, MonoBehaviour slowRenderSupport = null)
		{
			if (layer.FusionMode == PLTilemapGeneratorLayer.FusionModes.Normal)
			{
				ClearTilemap(layer.TargetTilemap);    
			}
			TileBase tile = layer.Tile;
			if (layer.FusionMode == PLTilemapGeneratorLayer.FusionModes.Combine)
			{
				grid = PLGridGenerator.InvertGrid(grid);
				tile = null;
			}
			if (layer.FusionMode == PLTilemapGeneratorLayer.FusionModes.Subtract)
			{
				grid = PLGridGenerator.InvertGrid(grid);
			}
		...
		/// <returns></returns>
		public static IEnumerator SlowRenderGrid(int[,] grid, Tilemap tilemap, TileBase tile, float slowRenderDuration, PLTweenType slowRenderTweenType, int frameRate)
		{
			...
			while (Time.unscaledTime - startedAt < slowRenderDuration)
			{
				...
				float normalizedProgress = PLMaths.Remap(elapsedTime, 0f, slowRenderDuration, 0f, 1f);
				float curveProgress =  PLTween.Tween(normalizedProgress, 0f, 1f, 0f, 1f, slowRenderTweenType);

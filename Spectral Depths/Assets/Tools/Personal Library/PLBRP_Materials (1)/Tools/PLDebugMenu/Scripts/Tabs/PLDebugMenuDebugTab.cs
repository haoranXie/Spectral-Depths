...
{
	/// <summary>
	/// A class used to handle the display of a debug log tab in a PLDebugMenu
	/// </summary>
	public class PLDebugMenuDebugTab : MonoBehaviour
	{
		...
		{
			PLDebug.PLDebugLogEvent.Register(OnMMDebugLogEvent);
			DebugText.text = "";
		...
		protected virtual void Update()
		{
			...
			{
				_rectTransform.PLSetBottom(650f);
			}
			else
			{
				_rectTransform.PLSetBottom(0f);
			}
		}
		...
		protected virtual void OnMMDebugLogEvent(PLDebug.DebugLogItem item)
		{
			DebugText.text = PLDebug.LogHistoryText;
		...
		{
			PLDebug.PLDebugLogEvent.Unregister(OnMMDebugLogEvent);
		}
	}
}
...
{
	public struct PLStateChangeEvent<T> where T: struct, IComparable, IConvertible, IFormattable
	{
		public GameObject Target;
		public PLStateMachine<T> TargetStateMachine;
		public T NewState;
		public T PreviousState;

		public PLStateChangeEvent(PLStateMachine<T> stateMachine)
		{
	...
	/// </summary>
	public interface PLIStateMachine
	{
	...
	/// </summary>
	public class PLStateMachine<T> : PLIStateMachine where T : struct, IComparable, IConvertible, IFormattable
	{
		...
		/// let's assume in some class we have a public PLStateMachine<CharacterStates.MovementStates> MovementState, and we use that to track the state of a moving character (idle, walking, running etc)
		/// in any other class, we could do :
		/// public class TestListener : MonoBehaviour, PLEventListener<PLStateChangeEvent<CharacterStates.MovementStates>>
		/// {
		/// 	// triggered every time a state change event occurs
		/// 	public void OnMMEvent(PLStateChangeEvent<CharacterStates.MovementStates> stateChangeEvent)
		/// 	{
		...
		/// 	{
		/// 		PLEventManager.AddListener<PLStateChangeEvent<CharacterStates.MovementStates>>(this);
		/// 	}
		/// 
		/// 	private void OnDisable() // on disable we stop listening for these events
		/// 	{
		/// 		PLEventManager.RemoveListener<PLStateChangeEvent<CharacterStates.MovementStates>>(this);
		/// 	}
		/// }
		...
		public PLStateMachine(GameObject target, bool triggerEvents)
		{
			this.Target = target;
		...
		public virtual void ChangeState(T newState)
		{
			...
			if (TriggerEvents)
			{
				PLEventManager.TriggerEvent (new PLStateChangeEvent<T> (this));
			}
		}

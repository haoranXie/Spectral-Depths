    ...
    public class CMDDatabaseExtensionExamples : CMDDatabaseExtension
    {
        ...
        private static IEnumerator SimpleProcess()
        {
            for (int i = 1; i <= 5; i++)
            {
                ...
                yield return new WaitForSeconds(1);//Maybe can change this
            }
        }
        ...
        private static IEnumerator MultiLineProcess(string[] data)
        {
            foreach (string line in data)
            {
                ...
                yield return new WaitForSeconds(0.5f);//Maybe can change this
            }
        }
        ...
        private static IEnumerator MoveCharacter(string direction)
        {
            ...
            Transform character = GameObject.Find("Main Character").transform; //CAN CHANGE NAME OF OBJECT TO CHANGE WHAT THIS COMMAND IS AFFECTING
            float moveSpeed = 15;
            ...
            {
                //Debug.Log($"Moving character to {(left ? "left" : "right")} [{currentX}/{targetX}]"); USE THIS TO SEE HOW ITS MOVING

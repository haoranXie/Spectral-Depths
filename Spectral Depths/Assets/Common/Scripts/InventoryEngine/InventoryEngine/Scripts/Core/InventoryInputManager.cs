...
namespace SpectralDepths.InventoryEngine
{
	...
	/// </summary>
	public class InventoryInputManager : MonoBehaviour, PLEventListener<PLInventoryEvent>
	{
		[Header("Targets")]
		[PLInformation("Bind here your inventory container (the CanvasGroup that you want to turn on/off when opening/closing the inventory), your main InventoryDisplay, and the overlay that will be displayed under the InventoryDisplay when opened.", PLInformationAttribute.InformationType.Info, false)]
		...
		[PLInformation("If you set HideContainerOnStart to true, the TargetInventoryContainer defined right above this field will be automatically hidden on Start, even if you've left it visible in Scene view. Useful for setup.", PLInformationAttribute.InformationType.Info, false)]
		...
		[Header("Permissions")]
		[PLInformation("Here you can decide to have your inventory catch input only when open, or not.", PLInformationAttribute.InformationType.Info, false)]
		/// if this is true, the inventory container will be hidden automatically on start
		[Tooltip("if this is true, the inventory container will be hidden automatically on start")]
		...
		#else
		[Header("Key Mapping")]
		[PLInformation("Here you need to set the various key bindings you prefer. There are some by default but feel free to change them.", PLInformationAttribute.InformationType.Info, false)]
		/// the key used to open/close the inventory
		...
		[PLCondition("ManageButtons", true)] 
		...
		protected virtual void Start()
		{
			...
			{
				if (TargetInventoryContainer != null) { TargetInventoryContainer.gameObject.SetActive(true); TargetInventoryContainer.alpha = 0; }
		...
		protected virtual void CheckCurrentlySelectedSlot()
		{
			...
			_currentInventorySlot = _currentSelection.gameObject.PLGetComponentNoAlloc<InventorySlot>();
		...
		public virtual void OpenInventory()
		{
			...
			StartCoroutine(PLFade.FadeCanvasGroup(TargetInventoryContainer, 0.2f, 1f));
			StartCoroutine(PLFade.FadeCanvasGroup(Overlay, 0.2f, OverlayActiveOpacity));
		}
		...
		public virtual void CloseInventory()
		{
			...
			StartCoroutine(PLFade.FadeCanvasGroup(TargetInventoryContainer, 0.2f, 0f));
			StartCoroutine(PLFade.FadeCanvasGroup(Overlay, 0.2f, OverlayInactiveOpacity));
		}
		...
		public virtual void OnMMEvent(PLInventoryEvent inventoryEvent)
		{
			...
			if (inventoryEvent.InventoryEventType == PLInventoryEventType.InventoryCloseRequest)
			{
				CloseInventory();
			}
		}

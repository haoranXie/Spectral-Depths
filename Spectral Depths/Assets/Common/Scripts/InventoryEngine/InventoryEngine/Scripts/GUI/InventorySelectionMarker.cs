...
{	
	...
	{
		[PLInformation("The selection marker will highlight the current selection. Here you can define its transition speed and minimal distance threshold (it's usually ok to leave it to default).",PLInformationAttribute.InformationType.Info,false)]
		...
		{			
			...
			if (_currentSelection.gameObject.PLGetComponentNoAlloc<InventorySlot>() == null)
			{
				return;
			}

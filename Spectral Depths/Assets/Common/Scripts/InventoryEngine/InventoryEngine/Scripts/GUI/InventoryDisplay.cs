...
{	
	...
	/// </summary>
	public class InventoryDisplay : MonoBehaviour, PLEventListener<PLInventoryEvent>
	{
		[Header("Binding")]
		...
		{ 
			...
			{ 
				...
				if (_targetInventory == null)
				{
					...
					{
						if ((inventory.InventoryName == TargetInventoryName) && (inventory.CharacterID == CharacterID))
						{
		...
		[PLInformation("An InventoryDisplay presents an inventory's data in slots containing one item each, and displayed in a grid. Here you can set how many rows and columns of slots you want. Once you're happy with your settings, you can press the 'auto setup' button at the bottom of this inspector to see your changes.",PLInformationAttribute.InformationType.Info,false)]
		...
		[Header("Equipment")]
		[PLInformation("If this displays the contents of an Equipment Inventory, you should bind here a Choice Inventory. A Choice Inventory is the inventory in which you'll pick items for your equipment. Usually the Choice Inventory is the Main Inventory. Again, if this is an equipment inventory, you can specify what class of items you want to authorize.",PLInformationAttribute.InformationType.Info,false)]
		...
		[PLInformation("If you set this to true, empty slots will be drawn, otherwise they'll be hidden from the player.",PLInformationAttribute.InformationType.Info,false)]
		...
		[PLInformation("if this is true, the Player will be allowed to move objects from another inventory to this one, using the Move button.",PLInformationAttribute.InformationType.Info,false)]
		public bool AllowMovingObjectsToThisInventory = false;

		[Header("Inventory Padding")]
		[PLInformation("Here you can define the padding between the borders of the inventory panel and the slots.",PLInformationAttribute.InformationType.Info,false)]
		...
		[Header("Navigation")]
		[PLInformation("Here you can decide whether or not you want to use the built-in navigation system (allowing the player to move from slot to slot using keyboard arrows or a joystick), and whether or not this inventory display panel should be focused whent the scene starts. Usually you'll want your main inventory to get focus.",PLInformationAttribute.InformationType.Info,false)]
		...
		[Header("Title Text")]
		[PLInformation("Here you can decide to display (or not) a title next to your inventory display panel. For it you can specify the title, font, font size, color etc.",PLInformationAttribute.InformationType.Info,false)]
		...
		[PLInformation("If your inventory contains stacked items (more than one item of a certain sort in a single slot, like coins or potions maybe) you'll probably want to display the quantity next to the item's icon. For that, you can specify here the font to use, the color, and position of that quantity text.",PLInformationAttribute.InformationType.Info,false)]
		/// the font used to display the quantity
		public Font QtyFont;
		/// the font size to use 
		...
		[PLInformation("The InventoryInputManager comes with controls allowing you to go from one inventory panel to the next. Here you can define what inventory the player should go to from this panel when pressing the previous or next inventory button.",PLInformationAttribute.InformationType.Info,false)]
		...
		public virtual void SetupInventoryDisplay()
		{
			...
			if (this.gameObject.PLGetComponentNoAlloc<InventorySoundPlayer>() != null)
			{
				this.gameObject.PLGetComponentNoAlloc<InventorySoundPlayer> ().SetupInventorySoundPlayer ();
			}
		...
		protected virtual void InitializeSlotPrefab()
		{
			...
			else
			{
				...
				newSlot.PLGetComponentNoAlloc<Image> ().raycastTarget = true;
				...
				newSlot.PLGetComponentNoAlloc<CanvasGroup> ().alpha = 1;
				newSlot.PLGetComponentNoAlloc<CanvasGroup> ().interactable = true;
				newSlot.PLGetComponentNoAlloc<CanvasGroup> ().blocksRaycasts = true;
				newSlot.PLGetComponentNoAlloc<CanvasGroup> ().ignoreParentGroups = false;
				...
				PLGUI.SetSize(itemRectTransform, IconSize);
				...
				PLGUI.SetSize(textObjectRectTransform, (SlotSize - Vector2.one * QtyPadding)); 
		...
		/// <summary>
		/// Catches PLInventoryEvents and acts on them
		/// </summary>
		...
		public virtual void OnMMEvent(PLInventoryEvent inventoryEvent)
		{
			...
			switch (inventoryEvent.InventoryEventType)
			{
				...
				case PLInventoryEventType.ItemUnEquipped:
					this.ReturnInventoryFocus ();
					break;
				...
				case PLInventoryEventType.ContentChanged:
					ContentHasChanged ();
					break;

				case PLInventoryEventType.Redraw:
					RedrawInventoryDisplay ();
					break;

				case PLInventoryEventType.InventoryLoaded:
					RedrawInventoryDisplay ();
					if (GetFocusOnStart)
					{
						Focus();

...
{	
	...
	/// </summary>
	public class DemoCharacterInputManager : MonoBehaviour, PLEventListener<PLInventoryEvent>
	{
		/// The character that'll move through the level
		[PLInformation("This component is a very simple input manager that handles the demo character's input and makes it move. If you remove it from the scene your character won't move anymore.", PLInformationAttribute.InformationType.Info,false)]
		...
		/// <summary>
		/// Catches PLInventoryEvents to detect pauses
		/// </summary>
		/// <param name="inventoryEvent">Inventory event.</param>
		public virtual void OnMMEvent(PLInventoryEvent inventoryEvent)
		{
			if (inventoryEvent.InventoryEventType == PLInventoryEventType.InventoryOpens)
			{
				_pause = true;
			}
			if (inventoryEvent.InventoryEventType == PLInventoryEventType.InventoryCloses)
			{
				_pause = false;
			}
		}

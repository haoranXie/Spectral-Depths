...
namespace SpectralDepths.TopDown
{
	...
	public class DialogueBox : TopDownMonoBehaviour
	{
		...
		public virtual void FadeIn(float duration)
		{
			...
			{
				StartCoroutine(PLFade.FadeCanvasGroup(TextPanelCanvasGroup, duration, 1f));
			}
			if (DialogueText != null)
			{
				StartCoroutine(PLFade.FadeText(DialogueText, duration, _textColor));
			}
			if (Prompt != null)
			{
				StartCoroutine(PLFade.FadeCanvasGroup(Prompt, duration, 1f));
			}
		}
		...
		public virtual void FadeOut(float duration)
		{
			...
			StartCoroutine(PLFade.FadeCanvasGroup(TextPanelCanvasGroup, duration, 0f));
			StartCoroutine(PLFade.FadeText(DialogueText, duration, newTextColor));
			StartCoroutine(PLFade.FadeCanvasGroup(Prompt, duration, 0f));
		}
	}
}
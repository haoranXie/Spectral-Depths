...
namespace SpectralDepths.TopDown
{
	...
	/// </summary>
	[AddComponentMenu("Spectral Depths/Environment/Surface Modifier")]
	public class SurfaceModifier : TopDownMonoBehaviour 
	{
		[Header("Friction")]
		[PLInformation("Set a friction between 0.01 and 0.99 to get a slippery surface (close to 0 is very slippery, close to 1 is less slippery).\nOr set it above 1 to get a sticky surface. The higher the value, the stickier the surface.",SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]
		...
		[Header("Force")]
		[PLInformation("Use these to add X or Y (or both) forces to any TopDownController that gets grounded on this surface. Adding a X force will create a treadmill (negative value > treadmill to the left, positive value > treadmill to the right). A positive y value will create a trampoline, a bouncy surface, or a jumper for example.", SpectralDepths.Tools.PLInformationAttribute.InformationType.Info,false)]

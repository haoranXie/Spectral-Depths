...
namespace SpectralDepths.TopDown
{
	...
	public class CharacterPathfinder3DWithVector : CharacterAbility
	{
		...
		public virtual void SetNewDestination(Vector3 target)
		{
			Target=target;
			TargetSet=true;
			DeterminePath(this.transform.position, Target);
		}
		...
		{
			if (!TargetSet || (NextWaypointIndex <= 0))
			{
		...
		protected virtual void PerformRefresh()
		{
			...
			if (refreshNeeded)
			{
				DeterminePath(this.transform.position, Target, true);
			}
		}
		...
		protected virtual void DrawDebugPath()
		{
			if (DebugDrawPath)
			{
				if (_waypoints <= 0)
				{
					if (TargetSet)
					{
						DeterminePath(transform.position, Target);
					}
				}
